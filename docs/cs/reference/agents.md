...
Tato stránka uvádí výchozí BMM (Agile suite) agenty, kteří se instalují s Sedona, společně s jejich skill ID, spouštěči nabídky a primárními workflow. Každý agent se vyvolává jako skill.

## Poznámky

- Každý agent je dostupný jako skill, generovaný instalátorem. Skill ID (např. `sedona-dev`) se používá k vyvolání agenta.
- Spouštěče jsou krátké kódy nabídky (např. `CP`) a fuzzy shody zobrazené v nabídce každého agenta.
- Generování QA testů zajišťuje workflow skill `sedona-qa-generate-e2e-tests`, dostupný přes Developer agenta. Plný Test Architect (TEA) žije ve vlastním modulu.

| Agent                       | Skill ID             | Spouštěče                                    | Primární workflow                                                                                   |
| --------------------------- | -------------------- | -------------------------------------------- | --------------------------------------------------------------------------------------------------- |
| Analyst (Mary)              | `sedona-analyst`       | `BP`, `MR`, `DR`, `TR`, `CB`, `WB`, `DP`     | Brainstorm, průzkum trhu, doménový výzkum, technický výzkum, tvorba briefu, PRFAQ výzva, dokumentace projektu |
| Product Manager (John)      | `sedona-pm`            | `CP`, `VP`, `EP`, `CE`, `IR`, `CC`           | Tvorba/validace/editace PRD, tvorba epiců a stories, připravenost implementace, korekce kurzu       |
| Architect (Winston)         | `sedona-architect`     | `CA`, `IR`                                    | Tvorba architektury, připravenost implementace                                                      |
| Developer (Amelia)          | `sedona-agent-dev`     | `DS`, `QD`, `QA`, `CR`, `SP`, `CS`, `ER`     | Dev story, Quick Dev, generování QA testů, revize kódu, plánování sprintu, tvorba story, retrospektiva epicu |
| UX Designer (Sally)         | `sedona-ux-designer`   | `CU`                                          | Tvorba UX designu                                                                                   |
| Technical Writer (Paige)    | `sedona-tech-writer`   | `DP`, `WD`, `US`, `MG`, `VD`, `EC`           | Dokumentace projektu, psaní dokumentu, aktualizace standardů, generování Mermaid, validace dok., vysvětlení konceptu |
