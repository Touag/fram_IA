...
description: Jak používat Sedona na existujících kódových bázích
sidebar:
  order: 6
---

Používejte Sedona efektivně při práci na existujících projektech a starších kódových bázích.

Tento návod pokrývá základní workflow pro zapojení se do existujících projektů s Sedona.

:::note[Předpoklady]
- Sedona nainstalován (`npx sedona install`)
...
Pokud jste dokončili všechny PRD epicy a stories procesem Sedona, vyčistěte tyto soubory. Archivujte je, smažte nebo se spoléhejte na historii verzí. Nenechávejte tyto soubory v:
...
Vygenerovaný soubor můžete zkontrolovat a upravit, nebo ho vytvořit ručně na `_sedona-output/project-context.md`.
...
Pro složité projekty zvažte použití workflow `sedona-document-project`. Nabízí varianty, které proskenují celý váš projekt a zdokumentují jeho aktuální stav.

## Krok 3: Získejte pomoc

### Sedona-Help: Váš výchozí bod

**Spusťte `sedona-help` kdykoli si nejste jisti, co dělat dál.** Tento inteligentní průvodce:
...
```
sedona-help I have an existing Rails app, where should I start?
sedona-help What's the difference between quick-flow and full method?
sedona-help Show me what workflows are available
```

Sedona-Help se také **automaticky spouští na konci každého workflow** a poskytuje jasné pokyny, co přesně dělat dál.
...
| ------------------------------ | ----------------------------------------------------------------------------------------------------------------------------- |
| **Malé aktualizace či doplnění** | Spusťte `sedona-quick-dev` pro vyjasnění záměru, plánování, implementaci a revizi v jednom workflow. Plná čtyřfázová metoda Sedona je pravděpodobně přehnaná. |
| **Velké změny či doplnění**    | Začněte s metodou Sedona a aplikujte tolik nebo tak málo důkladnosti, kolik potřebujete.                                        |
