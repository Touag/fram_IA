...
- `sedona-create-architecture` — respektuje technické preference během solutioningu
- `sedona-create-story` — informuje tvorbu stories vzory projektu
- `sedona-dev-story` — vede implementační rozhodnutí
- `sedona-code-review` — validuje proti standardům projektu
- `sedona-quick-dev` — aplikuje vzory při implementaci specifikací
- `sedona-sprint-planning`, `sedona-retrospective`, `sedona-correct-course` — poskytuje celkový kontext projektu
...
| ------------------------------------ | ----------------------------------------------- | -------------------------------------------------------------------- |
| **Nový projekt, před architekturou** | Ručně, před `sedona-create-architecture`          | Dokumentujte vaše technické preference, aby je architekt respektoval |
| **Nový projekt, po architektuře**    | Přes `sedona-generate-project-context` nebo ručně | Zachyťte architektonická rozhodnutí pro implementační agenty         |
| **Existující projekt**               | Přes `sedona-generate-project-context`            | Objevte existující vzory, aby agenti dodržovali zavedené konvence    |
| **Quick Flow projekt**               | Před nebo během `sedona-quick-dev`                | Zajistěte, aby rychlá implementace respektovala vaše vzory           |
...
Vytvořte soubor na `_sedona-output/project-context.md` a přidejte svá pravidla:
...
Spusťte workflow `sedona-generate-project-context` po dokončení architektury:
...
Pro existující projekty spusťte `sedona-generate-project-context` pro objevení existujících vzorů:
...
Můžete ho kdykoli ručně upravit, nebo přegenerovat `sedona-generate-project-context` po významných změnách.

:::note[Umístění souboru]
Výchozí umístění je `_sedona-output/project-context.md`. Workflow ho tam hledají a také kontrolují `**/project-context.md` kdekoli ve vašem projektu.
:::
