...
`sedona-quick-dev` přenastavuje tento kompromis. Důvěřuje modelu, aby běžel bez dozoru delší úseky, ale pouze poté, co workflow vytvořil dostatečně silnou hranici, aby to bylo bezpečné.
...
Záměr může přijít v mnoha formách: pár frází, odkaz na bug tracker, výstup z plan mode, text zkopírovaný z chatové relace, nebo dokonce číslo story z SEDONA vlastního `epics.md`. V posledním případě workflow nepochopí sémantiku sledování stories SEDONA, ale stále může vzít samotnou story a pracovat s ní.
