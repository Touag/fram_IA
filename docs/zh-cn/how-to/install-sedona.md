---
title: "如何安装 Sedona"
description: 在项目中安装 Sedona 的分步指南
sidebar:
  order: 1
---

使用 `npx sedona install` 在项目中安装 Sedona，并按需选择模块和 AI 工具。
...
:::note[启用 Skills]
某些平台需要你在设置中手动启用 skills 才会显示。如果你已经安装 Sedona 但看不到 skills，请检查平台设置，或直接询问你的 AI 助手如何启用 skills。
:::

### 4. 选择模块

安装程序会显示可用的模块。选择你需要的模块——大多数用户只需要 **Sedona**（软件开发模块）。
...
运行 `sedona-help` 来验证一切正常并查看下一步操作。

**Sedona-Help 是你的智能向导**，它会：
- 确认你的安装正常工作
- 根据你安装的模块显示可用内容
- 推荐你的第一步
...
你也可以向它提问：
```
sedona-help 我刚安装完成，应该先做什么？
sedona-help 对于 SaaS 项目我有哪些选项？
```
...
**安装程序工作正常但后续出现问题**——你的 AI 需要 Sedona 上下文才能提供帮助。请参阅[如何获取关于 Sedona 的答案](./get-answers-about-sedona.md)了解如何将你的 AI 指向正确的来源。
