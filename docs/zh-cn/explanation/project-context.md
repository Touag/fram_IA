...
**常见加载方包括：**
- `sedona-create-architecture`：在 solutioning 时纳入你的技术偏好
- `sedona-create-story`：按项目约定拆分和描述 story
- `sedona-dev-story`：约束实现路径和代码风格
- `sedona-code-review`：按项目标准做一致性校验
- `sedona-quick-dev`：在快速实现中避免偏离既有模式
- `sedona-sprint-planning`、`sedona-retrospective`、`sedona-correct-course`：读取项目级背景

## 什么时候建立或更新

| 场景 | 建议时机 | 目标 |
|----------|----------------|---------|
| **新项目（架构前）** | 在 `sedona-create-architecture` 前手动创建 | 先声明技术偏好，避免架构偏航 |
| **新项目（架构后）** | 通过 `sedona-generate-project-context` 生成并补充 | 把架构决策转成可执行规则 |
| **既有项目** | 先生成，再人工校对 | 让智能体学习现有约定而非重造体系 |
| **Quick Flow 场景** | 在 `sedona-quick-dev` 前或过程中维护 | 弥补跳过完整规划带来的上下文缺口 |
...
默认位置是 `_sedona-output/project-context.md`。工作流优先在该位置查找，也会扫描项目内的 `**/project-context.md`。
