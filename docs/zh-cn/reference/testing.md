...
内置 QA workflow（`sedona-qa-generate-e2e-tests`）是 BMM 模块的一部分，通过 Developer 智能体调用。目标是用你现有测试栈快速落地测试，不要求额外配置。

**触发方式：**
- 菜单触发器：`QA`（通过 Developer 智能体）
- skill：`sedona-qa-generate-e2e-tests`
...
:::note[范围边界]
QA workflow 只负责”生成测试”。如需实现质量评审与故事验收，请配合代码审查 workflow（`CR` / `sedona-code-review`）。
:::
...
**安装：** `npx sedona install` 后选择 TEA 模块。
...
按 Sedona workflow-map，测试位于阶段 4（实施）：

1. epic 内逐个 story：开发（`DS` / `sedona-dev-story`）+ 代码审查（`CR` / `sedona-code-review`）
2. epic 完成后：用 `QA`（通过 Developer 智能体）或 TEA 的 Automate 统一生成/补齐测试
3. 最后执行复盘（`sedona-retrospective`）
