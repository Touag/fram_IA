...
description: Tài liệu tham chiếu cho mọi tác vụ và quy trình tích hợp sẵn có trong mọi bản cài Sedona mà không cần module bổ sung.
sidebar:
  order: 2
---

Mọi bản cài Sedona đều bao gồm một tập skill cốt lõi có thể dùng cùng với bất cứ việc gì bạn đang làm, các tác vụ và quy trình độc lập hoạt động xuyên suốt mọi dự án, mọi module và mọi giai đoạn. Chúng luôn có sẵn bất kể bạn cài những module tùy chọn nào.

:::tip[Lối đi nhanh]
Chạy bất kỳ công cụ cốt lõi nào bằng cách gõ tên skill của nó, ví dụ `sedona-help`, trong IDE của bạn. Không cần mở phiên agent trước.
:::
...
| --- | --- | --- |
| [`sedona-help`](#sedona-help) | Tác vụ | Nhận hướng dẫn có ngữ cảnh về việc nên làm gì tiếp theo |
| [`sedona-brainstorming`](#sedona-brainstorming) | Quy trình | Tổ chức các phiên brainstorming có tương tác |
| [`sedona-party-mode`](#sedona-party-mode) | Quy trình | Điều phối thảo luận nhóm nhiều agent |
| [`sedona-distillator`](#sedona-distillator) | Tác vụ | Nén tài liệu tối ưu cho LLM mà không mất thông tin |
| [`sedona-advanced-elicitation`](#sedona-advanced-elicitation) | Tác vụ | Đẩy đầu ra của LLM qua các vòng tinh luyện lặp |
| [`sedona-review-adversarial-general`](#sedona-review-adversarial-general) | Tác vụ | Rà soát hoài nghi để tìm chỗ thiếu và chỗ sai |
| [`sedona-review-edge-case-hunter`](#sedona-review-edge-case-hunter) | Tác vụ | Phân tích toàn bộ nhánh rẽ để tìm trường hợp biên chưa được xử lý |
| [`sedona-editorial-review-prose`](#sedona-editorial-review-prose) | Tác vụ | Biên tập câu chữ nhằm tăng độ rõ ràng khi giao tiếp |
| [`sedona-editorial-review-structure`](#sedona-editorial-review-structure) | Tác vụ | Biên tập cấu trúc — cắt, gộp và tổ chức lại |
| [`sedona-shard-doc`](#sedona-shard-doc) | Tác vụ | Tách file markdown lớn thành các phần có tổ chức |
| [`sedona-index-docs`](#sedona-index-docs) | Tác vụ | Tạo hoặc cập nhật mục lục cho toàn bộ tài liệu trong một thư mục |
...
- Bạn mới làm quen với Sedona và cần định hướng
...
**Đầu vào:** Truy vấn ngôn ngữ tự nhiên tùy chọn, ví dụ `sedona-help I have a SaaS idea, where do I start?`
...
**Điều phối thảo luận nhóm nhiều agent.** Công cụ này nạp toàn bộ agent Sedona đã cài và tạo một cuộc trao đổi tự nhiên, nơi mỗi agent đóng góp từ góc nhìn chuyên môn và cá tính riêng.
...
Hãy chạy cả `sedona-review-adversarial-general` và `sedona-review-edge-case-hunter` để có độ bao phủ trực giao. Adversarial review bắt lỗi về chất lượng và độ đầy đủ; edge case hunter bắt các đường đi chưa được xử lý.
:::
