---
title: Các skill
description: Tài liệu tham chiếu cho skill của Sedona — skill là gì, hoạt động ra sao và tìm ở đâu.
sidebar:
  order: 3
---

Skills là các prompt dựng sẵn để nạp agent, chạy workflow hoặc thực thi task bên trong IDE của bạn. Trình cài đặt Sedona sinh chúng từ các module bạn đã chọn tại thời điểm cài đặt. Nếu sau này bạn thêm, xóa hoặc thay đổi module, hãy chạy lại trình cài đặt để đồng bộ skills (xem [Khắc phục sự cố](#khắc-phục-sự-cố)).

## Skill So Với Trigger Trong Menu Agent

Sedona cung cấp hai cách để bắt đầu công việc, và chúng phục vụ những mục đích khác nhau.

| Cơ chế | Cách gọi | Điều xảy ra |
| --- | --- | --- |
| **Skill** | Gõ tên skill, ví dụ `sedona-help`, trong IDE | Nạp trực tiếp agent, chạy workflow hoặc thực thi task |
...
Khi bạn chạy `npx sedona install`, trình cài đặt sẽ đọc manifest của mọi module được chọn rồi tạo một skill cho mỗi agent, workflow, task và tool. Mỗi skill là một thư mục chứa file `SKILL.md`, hướng dẫn AI nạp file nguồn tương ứng và làm theo chỉ dẫn trong đó.
...
Tên thư mục quyết định tên skill trong IDE. Ví dụ thư mục `sedona-agent-dev/` sẽ đăng ký skill `sedona-agent-dev`.
...
Chạy `sedona-help` để nhận hướng dẫn có ngữ cảnh về bước tiếp theo.
...
| --- | --- | --- |
| `sedona-agent-dev` | Amelia (Developer) | Triển khai story với mức tuân thủ đặc tả nghiêm ngặt |
| `sedona-pm` | John (Product Manager) | Tạo và kiểm tra PRD |
| `sedona-architect` | Winston (Architect) | Thiết kế kiến trúc hệ thống |
...
| --- | --- |
| `sedona-product-brief` | Tạo product brief — phiên discovery có hướng dẫn khi concept của bạn đã rõ |
| `sedona-prfaq` | Bài kiểm tra [Working Backwards PRFAQ](../explanation/analysis-phase.md#prfaq-working-backwards) để stress-test concept sản phẩm |
| `sedona-create-prd` | Tạo Product Requirements Document |
| `sedona-create-architecture` | Thiết kế kiến trúc hệ thống |
| `sedona-create-epics-and-stories` | Tạo epics và stories |
| `sedona-dev-story` | Triển khai một story |
| `sedona-code-review` | Chạy code review |
| `sedona-quick-dev` | Luồng nhanh hợp nhất — làm rõ yêu cầu, lập kế hoạch, triển khai, review và trình bày |
...
**Sedona-Help: người dẫn đường thông minh của bạn**

`sedona-help` là giao diện chính để bạn khám phá nên làm gì tiếp theo. Nó kiểm tra dự án, hiểu truy vấn ngôn ngữ tự nhiên và đề xuất bước bắt buộc hoặc tùy chọn tiếp theo dựa trên các module đã cài.
...
Mọi skill đều dùng tiền tố `sedona-` theo sau là tên mô tả, ví dụ `sedona-agent-dev`, `sedona-create-prd`, `sedona-help`. Xem [Modules](./modules.md) để biết các module hiện có.
...
**Thiếu skill mà bạn mong đợi.** Trình cài đặt chỉ tạo skill cho những module bạn đã chọn. Hãy chạy lại `npx sedona install` và kiểm tra lại phần chọn module. Đồng thời xác nhận rằng file skill thực sự tồn tại trong thư mục dự kiến.
