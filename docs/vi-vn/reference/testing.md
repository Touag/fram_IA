...
Sedona cung cấp hai hướng kiểm thử: workflow QA tích hợp sẵn để tạo test nhanh và module Test Architect có thể cài thêm cho chiến lược kiểm thử c��p doanh nghiệp.
...
| **Thiết lập** | Không cần cài thêm, đã có sẵn trong BMM | Cài riêng qua `npx sedona install` |
...
Workflow QA tích hợp sẵn (`sedona-qa-generate-e2e-tests`) nằm trong module BMM (Agile suite), khả dụng thông qua Developer agent. Nó tạo test chạy được rất nhanh bằng framework kiểm thử hiện có của dự án, không cần thêm cấu hình hay bước cài đặt bổ sung.

**Trigger:** `QA` (thông qua Developer agent) hoặc `sedona-qa-generate-e2e-tests`
...
- **Tài liệu:** [TEA Module Docs](https://sedona-code-org.github.io/sedona-test-architecture-enterprise/)
- **Cài đặt:** `npx sedona install` rồi chọn module TEA
...
Workflow QA Automate xuất hiện ở Phase 4 (Implementation) trong workflow map của Sedona. Nó được thiết kế để chạy **sau khi hoàn tất trọn vẹn một epic** — tức là khi mọi story trong epic đó đã được triển khai và code review xong. Trình tự điển hình là:
...
3. Chạy retrospective (`sedona-retrospective`) để ghi nhận bài học rút ra
