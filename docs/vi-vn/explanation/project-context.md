...
- `sedona-create-architecture` - tôn trọng ưu tiên kỹ thuật trong giai đoạn solutioning
- `sedona-create-story` - đưa pattern của dự án vào quá trình tạo story
- `sedona-dev-story` - định hướng các quyết định triển khai
- `sedona-code-review` - đối chiếu với tiêu chuẩn của dự án
- `sedona-quick-dev` - áp dụng pattern khi triển khai các spec
- `sedona-sprint-planning`, `sedona-retrospective`, `sedona-correct-course` - cung cấp bối cảnh cấp dự án
...
|----------|----------------|---------|
| **Dự án mới, trước kiến trúc** | Tạo thủ công, trước `sedona-create-architecture` | Ghi lại ưu tiên kỹ thuật để architect tôn trọng |
| **Dự án mới, sau kiến trúc** | Qua `sedona-generate-project-context` hoặc tạo thủ công | Ghi lại quyết định kiến trúc cho các agent triển khai |
| **Dự án hiện có** | Qua `sedona-generate-project-context` | Khám phá pattern hiện có để agent theo đúng quy ước |
| **Dự án Quick Flow** | Trước hoặc trong `sedona-quick-dev` | Đảm bảo triển khai nhanh vẫn tôn trọng pattern của bạn |
...
Tạo tệp tại `_sedona-output/project-context.md` và thêm các quy tắc của bạn:
...
Chạy workflow `sedona-generate-project-context` sau khi bạn hoàn tất kiến trúc:
...
Với dự án hiện có, chạy `sedona-generate-project-context` để khám phá pattern sẵn có:
...
Bạn có thể sửa thủ công bất kỳ lúc nào, hoặc chạy lại `sedona-generate-project-context` để cập nhật sau các thay đổi lớn.

:::note[Vị trí tệp]
Vị trí mặc định là `_sedona-output/project-context.md`. Các workflow tìm tệp ở đó, đồng thời cũng kiểm tra `**/project-context.md` ở bất kỳ đâu trong dự án.
:::
