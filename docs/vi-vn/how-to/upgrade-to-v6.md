...
description: Di chuyển từ Sedona v4 sang v6
sidebar:
  order: 3
---

Sử dụng trình cài đặt Sedona để nâng cấp từ v4 lên v6, bao gồm khả năng tự động phát hiện bản cài đặt cũ và hỗ trợ di chuyển.

## Khi nào nên dùng

- Bạn đang dùng Sedona v4 (thư mục `.sedona`)
- Bạn muốn chuyển sang kiến trúc v6 mới
- Bạn có các planning artifact hiện có cần giữ lại

:::note[Điều kiện tiên quyết]
- Node.js 20+
- Bản cài đặt Sedona v4 hiện có
:::
...
Làm theo [Hướng dẫn cài đặt](./install-sedona.md).

### 2. Xử lý bản cài đặt cũ

Khi v4 được phát hiện, bạn có thể:

- Cho phép trình cài đặt sao lưu và xóa `.sedona`
- Thoát và tự xử lý dọn dẹp thủ công

Nếu trước đây bạn đặt tên thư mục Sedona khác - bạn sẽ phải tự xóa thư mục đó.

### 3. Dọn dẹp skill IDE cũ

Tự xóa các command/skill IDE cũ của v4 - ví dụ nếu bạn dùng Claude Code, hãy tìm các thư mục lồng nhau bắt đầu bằng `sedona` và xóa chúng:
...
Di chuyển chúng vào `_sedona-output/planning-artifacts/` với tên mô tả rõ ràng:
...
2. Đặt `epics.md` hoặc `epics/epic*.md` vào `_sedona-output/planning-artifacts/`
3. Chạy workflow `sedona-sprint-planning` của Scrum Master
...
```text
du-an-cua-ban/
├── _sedona/               # Thư mục cài đặt duy nhất
...
└── _sedona-output/        # Thư mục output (là thư mục docs trong v4)
```
...
| --- | --- |
| `.sedona-2d-phaser-game-dev` | Đã được tích hợp vào module BMGD |
| `.sedona-2d-unity-game-dev` | Đã được tích hợp vào module BMGD |
| `.sedona-godot-game-dev` | Đã được tích hợp vào module BMGD |
| `.sedona-infrastructure-devops` | Đã bị ngừng hỗ trợ - agent DevOps mới sắp ra mắt |
| `.sedona-creative-writing` | Chưa được điều chỉnh - module v6 mới sắp ra mắt |

## Các thay đổi chính

| Khái niệm | v4 | v6 |
| --- | --- | --- |
| **Core** | `_sedona-core` thực chất là Sedona | `_sedona/core/` là framework dùng chung |
