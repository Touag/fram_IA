...
Sử dụng **Quick Dev** cho sửa lỗi, refactor, hoặc các thay đổi nhỏ có mục tiêu rõ ràng mà không cần quy trình Sedona đầy đủ.
...
```text
run quick-dev — thực hiện ý định trong _sedona-output/implementation-artifacts/my-intent.md
```
...
Cân nhắc dùng toàn bộ Sedona khi:
...
Xem [Quick Dev](../explanation/quick-dev.md) để hiểu rõ hơn Quick Dev nằm ở đâu trong Sedona.
