...
description: Cách sử dụng Sedona trên các codebase hiện có
sidebar:
  order: 6
---

Sử dụng Sedona hiệu quả khi làm việc với các dự án hiện có và codebase legacy.

Tài liệu này mô tả workflow cốt lõi để on-board vào các dự án đã tồn tại bằng Sedona.

:::note[Điều kiện tiên quyết]
- Đã cài Sedona (`npx sedona install`)
...
Nếu bạn đã hoàn thành toàn bộ epic và story trong PRD theo quy trình Sedona, hãy dọn dẹp những tệp đó. Bạn có thể lưu trữ, xóa đi, hoặc dựa vào lịch sử phiên bản nếu cần. Không nên giữ các tệp này trong:
...
Bạn có thể xem lại và chỉnh sửa tệp được tạo, hoặc tự tạo tệp tại `_sedona-output/project-context.md` nếu muốn.
...
Với các dự án phức tạp, hãy cân nhắc dùng workflow `sedona-document-project`. Nó có các biến thể lúc chạy có thể quét toàn bộ dự án và tài liệu hóa trạng thái thực tế hiện tại của hệ thống.

## Bước 4: Nhờ trợ giúp

### Sedona-Help: Điểm bắt đầu của bạn

**Hãy chạy `sedona-help` bất cứ lúc nào bạn không chắc cần làm gì tiếp theo.** Công cụ hướng dẫn thông minh này:
...
```text
sedona-help Tôi có một ứng dụng Rails đã tồn tại, tôi nên bắt đầu từ đâu?
sedona-help Điểm khác nhau giữa quick-flow và full method là gì?
sedona-help Cho tôi xem những workflow đang có
```

Sedona-Help cũng **tự động chạy ở cuối mỗi workflow**, đưa ra hướng dẫn rõ ràng về việc cần làm tiếp theo.
...
| --- | --- |
| **Cập nhật hoặc bổ sung nhỏ** | Chạy `sedona-quick-dev` để làm rõ ý định, lập kế hoạch, triển khai và review trong một workflow duy nhất. Quy trình Sedona đầy đủ có thể là quá mức cần thiết. |
| **Thay đổi hoặc bổ sung lớn** | Bắt đầu với Sedona, áp dụng mức độ chặt chẽ phù hợp với nhu cầu của bạn. |
