...
description: Migrer de Sedona v4 vers v6
sidebar:
  order: 3
---

Utilisez l'installateur Sedona pour passer de la v4 à la v6, qui inclut une détection automatique des installations existantes et une assistance à la migration.

## Quand utiliser ce guide

- Vous avez Sedona v4 installé (dossier `.sedona`)
- Vous souhaitez migrer vers la nouvelle architecture v6
- Vous avez des artefacts de planification existants à préserver

:::note[Prérequis]
- Node.js 20+
- Installation Sedona v4 existante
:::
...
Suivez les [Instructions d'installation](./install-sedona.md).

### 2. Gérer l'installation existante

Quand v4 est détecté, vous pouvez :

- Autoriser l'installateur à sauvegarder et supprimer `.sedona`
- Quitter et gérer le nettoyage manuellement

Si vous avez nommé votre dossier de méthode sedona autrement, vous devrez supprimer le dossier vous-même manuellement.

### 3. Nettoyer les skills IDE

Supprimez manuellement les commandes/skills IDE v4 existants - par exemple si vous avez Claude Code, recherchez tous les dossiers imbriqués qui commencent par sedona et supprimez-les :
...
Déplacez-les dans `_sedona-output/planning-artifacts/` avec des noms descriptifs :
...
2. Placez `epics.md` ou `epics/epic*.md`[^2] dans `_sedona-output/planning-artifacts/`
3. Lancez le workflow Développeur `sedona-sprint-planning`[^4]
...
```text
votre-projet/
├── _sedona/               # Dossier d'installation unique
...
└── _sedona-output/        # Dossier de sortie (était le dossier doc en v4)
```
...
| Module v4                     | Statut v6                                 |
| ----------------------------- | ----------------------------------------- |
| `.sedona-2d-phaser-game-dev`    | Intégré dans le Module BMGD               |
| `.sedona-2d-unity-game-dev`     | Intégré dans le Module BMGD               |
| `.sedona-godot-game-dev`        | Intégré dans le Module BMGD               |
| `.sedona-infrastructure-devops` | Déprécié - nouvel agent DevOps bientôt disponible |
| `.sedona-creative-writing`      | Non adapté - nouveau module v6 bientôt disponible   |

## Changements clés

| Concept       | v4                                    | v6                                   |
| ------------- | ------------------------------------- | ------------------------------------ |
| **Core**      | `_sedona-core` était en fait la méthode Sedona | `_sedona/core/` est le framework universel |
