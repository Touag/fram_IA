...
Utilisez **Quick Dev** pour les corrections de bugs, les refactorisations ou les petites modifications ciblées qui ne nécessitent pas la méthode Sedona complète.
...
```text
quick-dev — implémente _sedona-output/implementation-artifacts/my-intent.md
```
...
Envisagez d'utiliser la méthode Sedona complète lorsque :
...
Voir [Quick Dev](../explanation/quick-dev.md) pour plus d'informations sur la façon dont Quick Dev s'intègre dans la méthode Sedona.
