...
description: Comment utiliser la méthode Sedona sur des bases de code existantes
sidebar:
  order: 6
---

Utilisez la méthode Sedona efficacement lorsque vous travaillez sur des projets existants et des bases de code legacy.

Ce guide couvre le flux de travail essentiel pour l'intégration à des projets existants avec la méthode Sedona.

:::note[Prérequis]
- méthode Sedona installée (`npx sedona install`)
...
Si vous avez terminé tous les epics et stories du PRD[^1] via le processus Sedona, nettoyez ces fichiers. Archivez-les, supprimez-les, ou appuyez-vous sur l'historique des versions si nécessaire. Ne conservez pas ces fichiers dans :

- `docs/`
- `_sedona-output/planning-artifacts/`
- `_sedona-output/implementation-artifacts/`
...
```bash
sedona-generate-project-context
```
...
Vous pouvez examiner et affiner le fichier généré, ou le créer manuellement à `_sedona-output/project-context.md` si vous préférez.
...
Pour les projets complexes, envisagez d'utiliser le workflow `sedona-document-project`. Il offre des variantes d'exécution qui analyseront l'ensemble de votre projet et documenteront son état actuel réel.

## Étape 4 : Obtenir de l'aide

### Sedona-Help : Votre point de départ

**Exécutez `sedona-help` chaque fois que vous n'êtes pas sûr de la prochaine étape.** Ce guide intelligent :
...
```
sedona-help J'ai une app Rails existante, par où dois-je commencer ?
sedona-help Quelle est la différence entre quick-dev et la méthode complète ?
sedona-help Montre-moi quels workflows sont disponibles
```

Sedona-Help s'exécute également **automatiquement à la fin de chaque workflow**, fournissant des conseils clairs sur exactement quoi faire ensuite.
...
| ----------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------- |
| **Petites mises à jour ou ajouts**        | Exécutez `sedona-quick-dev` pour clarifier l'intention, planifier, implémenter et réviser dans un seul workflow. La méthode Sedona complète en quatre phases est probablement excessive. |
| **Modifications ou ajouts majeurs**       | Commencez avec la méthode Sedona, en appliquant autant ou aussi peu de rigueur que nécessaire.                                            |
