...
description: Installer Sedona en utilisant des options de ligne de commande pour les pipelines CI/CD et les déploiements automatisés
sidebar:
  order: 2
---

Utilisez les options de ligne de commande pour installer Sedona de manière non-interactive. Cela est utile pour :
...
| `--output-folder <chemin>` | Chemin du dossier de sortie (voir les règles de résolution ci-dessous) | `_sedona-output` |
...
|-------------------------------|----------------------------|--------------------------------------------------------------|
| Chemin relatif (par défaut)   | `_sedona-output`             | `<racine-du-projet>/_sedona-output`                            |
...
- `bmm` — méthode Sedona Master
- `bmb` — Sedona Builder

Consultez le [registre Sedona](https://github.com/sedona-code-org) pour les modules externes disponibles.
...
Exécutez `npx sedona install` de manière interactive une fois pour voir la liste complète actuelle des outils pris en charge, ou consultez la [configuration des codes de la plateforme](https://github.com/sedona-code-org/SEDONA/blob/main/tools/installer/ide/platform-codes.yaml).

## Modes d'installation

| Mode | Description | Exemple |
|------|-------------|---------|
| Entièrement non-interactif | Fournir toutes les options pour ignorer toutes les invites | `npx sedona install --directory . --modules bmm --tools claude-code --yes` |
| Semi-interactif | Fournir certains options ; Sedona demande les autres | `npx sedona install --directory . --modules bmm` |
| Paramètres par défaut uniquement | Accepter tous les paramètres par défaut avec `-y` | `npx sedona install --yes` |
| Sans outils | Ignorer la configuration des outils/IDE | `npx sedona install --modules bmm --tools none` |
...
- Un répertoire `_sedona/` entièrement configuré dans votre projet
- Des agents et des flux de travail configurés pour vos modules et outils sélectionnés
- Un dossier `_sedona-output/` pour les artefacts générés

## Validation et gestion des erreurs

Sedona valide toutes les options fournis :
...
Exécutez avec `--debug` pour une sortie détaillée, essayez le mode interactif pour isoler le problème, ou signalez-le à <https://github.com/sedona-code-org/SEDONA/issues>.
:::
