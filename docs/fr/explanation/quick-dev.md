...
`sedona-quick-dev` rééquilibre ce compromis. Il fait confiance au modèle pour s'exécuter sans surveillance sur de plus longues périodes, mais seulement après que le workflow ait créé une frontière suffisamment solide pour rendre cela sûr.
...
L'intention peut prendre plusieurs formes : quelques phrases, un lien vers un outil de suivi de bugs, une sortie du mode planification, du texte copié depuis une session de chat, ou même un numéro de story depuis un fichier `epics.md` de SEDONA. Dans ce dernier cas, le workflow ne comprendra pas la sémantique de suivi des stories de SEDONA, mais il peut quand même prendre la story elle-même et l'exécuter.
