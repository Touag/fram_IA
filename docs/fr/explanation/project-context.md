...
- `sedona-create-architecture` — respecte les préférences techniques pendant la phase de solutioning
- `sedona-create-story` — informe la création de stories avec les patterns du projet
- `sedona-dev-story` — guide les décisions d'implémentation
- `sedona-code-review` — valide par rapport aux standards du projet
- `sedona-quick-dev` — applique les patterns lors de l'implémentation des spécifications techniques
- `sedona-sprint-planning`, `sedona-retrospective`, `sedona-correct-course` — fournit le contexte global du projet
...
|------------------------------------------|-----------------------------------------------------|---------------------------------------------------------------------------------------|
| **Nouveau projet, avant l'architecture** | Manuellement, avant `sedona-create-architecture`      | Documenter vos préférences techniques pour que l'architecte les respecte              |
| **Nouveau projet, après l'architecture** | Via `sedona-generate-project-context` ou manuellement | Capturer les décisions d'architecture pour les agents d'implémentation                |
| **Projet existant**                      | Via `sedona-generate-project-context`                 | Découvrir les patterns existants pour que les agents suivent les conventions établies |
| **Projet Quick Dev**                   | Avant ou pendant `sedona-quick-dev`                   | Garantir que l'implémentation rapide respecte vos patterns                            |
...
Créez le fichier `_sedona-output/project-context.md` et ajoutez vos règles :

```bash
# Depuis la racine du projet
mkdir -p _sedona-output
touch _sedona-output/project-context.md
```
...
Exécutez le workflow `sedona-generate-project-context` après avoir terminé votre architecture :
...
Pour les projets existants, exécutez `sedona-generate-project-context` pour découvrir les patterns existants :
...
Vous pouvez l'éditer manuellement à tout moment, ou réexécuter `sedona-generate-project-context` pour le mettre à jour après des changements significatifs.

:::note[Emplacement du Fichier]
L'emplacement par défaut est `_sedona-output/project-context.md`. Les workflows le recherchent là, et vérifient également `**/project-context.md` n'importe où dans votre projet.
:::
