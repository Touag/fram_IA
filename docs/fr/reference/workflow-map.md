...
Si à tout moment vous ne savez pas quoi faire, le skill `sedona-help` vous aidera à rester sur la bonne voie ou à savoir quoi faire ensuite. Vous pouvez toujours vous référer à cette page également — mais `sedona-help` est entièrement interactif et beaucoup plus rapide si vous avez déjà installé la méthode Sedona. De plus, si vous utilisez différents modules qui ont étendu la méthode Sedona ou ajouté d'autres modules complémentaires non extensifs — `sedona-help` évolue pour connaître tout ce qui est disponible et vous donner les meilleurs conseils du moment.
...
<iframe src="/workflow-map-diagram-fr.html" title="Diagramme de la carte des workflows de la méthode Sedona" width="100%" height="100%" style="border-radius: 8px; border: 1px solid #334155; min-height: 900px;"></iframe>
...
|---------------------------------------------------------------------------|------------------------------------------------------------------------------------------|---------------------------|
| `sedona-brainstorming`                                                      | Brainstormez des idées de projet avec l’accompagnement guidé d’un coach de brainstorming | `brainstorming-report.md` |
| `sedona-domain-research`, `sedona-market-research`, `sedona-technical-research` | Validez les hypothèses de marché, techniques ou de domaine                               | Rapport de recherches     |
| `sedona-product-brief`                                                      | Capturez la vision stratégique — idéal lorsque votre concept est clair                   | `product-brief.md`        |
| `sedona-prfaq`                                                              | Working Backwards — éprouvez et forgez votre concept produit                             | `prfaq-{project}.md`       |
...
| Workflow                | Objectif                                                | Produit      |
|-------------------------|---------------------------------------------------------|--------------|
| `sedona-create-prd`       | Définissez les exigences (FRs/NFRs)[^1]                     | `PRD.md`[^2]     |
| `sedona-create-ux-design` | Concevez l'expérience utilisateur (lorsque l'UX compte) | `ux-spec.md` |
...
| Workflow                              | Objectif                                          | Produit                      |
|---------------------------------------|---------------------------------------------------|------------------------------|
| `sedona-create-architecture`            | Rendez les décisions techniques explicites        | `architecture.md` avec ADRs[^3]  |
| `sedona-create-epics-and-stories`       | Décomposez les exigences en travail implémentable | Fichiers d'epic avec stories |
| `sedona-check-implementation-readiness` | Vérification avant implémentation                 | Décision Passe/Réserves/Échec  |
...
| Workflow               | Objectif                                                                            | Produit                          |
|------------------------|-------------------------------------------------------------------------------------|----------------------------------|
| `sedona-sprint-planning` | Initialisez le suivi (une fois par projet pour séquencer le cycle de développement) | `sprint-status.yaml`             |
| `sedona-create-story`    | Préparez la story suivante pour implémentation                                      | `story-[slug].md`                |
| `sedona-dev-story`       | Implémentez la story                                                                | Code fonctionnel + tests         |
| `sedona-code-review`     | Validez la qualité de l'implémentation                                              | Approuvé ou changements demandés |
| `sedona-correct-course`  | Gérez les changements significatifs en cours de sprint                              | Plan mis à jour ou réorientation |
| `sedona-sprint-status`   | Suivez la progression du sprint et le statut des stories                            | Mise à jour du statut du sprint  |
| `sedona-retrospective`   | Revue après complétion d'un epic                                                    | Leçons apprises                  |
...
| Workflow         | Objectif                                                                            | Produit               |
|------------------|-------------------------------------------------------------------------------------|-----------------------|
| `sedona-quick-dev` | Flux rapide unifié — clarifie l'intention, planifie, implémente, révise et présente | `spec-*.md` + code |
...
- **Manuellement** — Créez `_sedona-output/project-context.md` avec votre pile technologique et vos règles d'implémentation
- **Générez-le** — Exécutez `sedona-generate-project-context` pour l'auto-générer à partir de votre architecture ou de votre codebase
