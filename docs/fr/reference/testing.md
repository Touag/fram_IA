...
Sedona propose deux approches de test : un workflow QA[^1] intégré pour une génération rapide de tests et un module Test Architect installable pour une stratégie de test de qualité entreprise.
...
| **Installation**        | Rien à installer — inclus dans BMM          | Installer séparément via `npx sedona install`                  |
...
Le workflow QA intégré (`sedona-qa-generate-e2e-tests`) fait partie du module BMM (suite Agile), disponible via l'agent Developer. Il génère rapidement des tests fonctionnels en utilisant le framework de test existant de votre projet — aucune configuration ni installation supplémentaire requise.

**Déclencheur :** `QA` (via l'agent Developer) ou `sedona-qa-generate-e2e-tests`
...
- **Documentation :** [TEA Module Docs](https://sedona-code-org.github.io/sedona-test-architecture-enterprise/)
- **Installation :** `npx sedona install` et sélectionnez le module TEA
...
Le workflow Automate du QA intégré apparaît dans la Phase 4 (Implémentation) de la carte de workflow méthode Sedona. Il est conçu pour s'exécuter **après qu'un epic complet soit terminé** — une fois que toutes les stories d'un epic ont été implémentées et revues. Une séquence typique :
...
3. Lancer la rétrospective (`sedona-retrospective`) pour capturer les leçons apprises
