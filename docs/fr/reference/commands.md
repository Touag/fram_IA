...
Le nom du répertoire détermine le nom du skill dans votre IDE. Par exemple, le répertoire `sedona-agent-dev/` enregistre le skill `sedona-agent-dev`.
...
Exécutez `sedona-help` pour obtenir des conseils contextuels sur votre prochaine étape.
...
|------------------|------------------------|-------------------------------------------------------------|
| `sedona-agent-dev` | Amelia (Développeur)   | Implémente les stories avec une adhérence stricte aux specs |
| `sedona-pm`        | John (Product Manager) | Crée et valide les PRDs[^1]                                 |
| `sedona-architect` | Winston (Architecte)   | Conçoit l'architecture système                              |
...
| --- | --- |
| `sedona-product-brief` | Créer un product brief[^3] — découverte guidée lorsque votre concept est clair |
| `sedona-prfaq` | Défi [PRFAQ Working Backwards](../explanation/analysis-phase.md#prfaq-working-backwards) pour éprouver votre concept produit |
| `sedona-create-prd` | Créer un PRD[^1] |
| `sedona-create-architecture` | Concevoir l'architecture système |
| `sedona-create-epics-and-stories` | Créer des epics et des stories |
| `sedona-dev-story` | Implémenter une story |
| `sedona-code-review` | Effectuer une revue de code |
| `sedona-quick-dev` | Flux rapide unifié — clarifier l'intention, planifier, implémenter, réviser, présenter |
...
**Sedona-Help : Votre guide intelligent**

`sedona-help` est votre interface principale pour découvrir quoi faire ensuite. Il inspecte votre projet, comprend les requêtes en langage naturel et recommande la prochaine étape requise ou optionnelle en fonction de vos modules installés.
...
Tous les skills utilisent le préfixe `sedona-` suivi d'un nom descriptif (ex. `sedona-agent-dev`, `sedona-create-prd`, `sedona-help`). Consultez [Modules](./modules.md) pour les modules disponibles.
...
**Des skills attendus sont manquants.** L'installateur génère uniquement les skills pour les modules que vous avez sélectionnés. Exécutez à nouveau `npx sedona install` et vérifiez votre sélection de modules. Vérifiez que les fichiers de skill existent dans le répertoire attendu.
...
[^2]: Persona : dans le contexte de Sedona, un persona désigne un agent IA avec un rôle défini, un style de communication et une expertise spécifiques (ex. Mary l'analyste, Winston l'architecte). Chaque persona garde son "caractère" pendant les interactions.
