---
title: Skills
description: Reference for Sedona skills — what they are, how they work, and where to find them.
sidebar:
  order: 3
---

Skills are pre-built prompts that load agents, run workflows, or execute tasks inside your IDE. The Sedona installer generates them from your installed modules at install time. If you later add, remove, or change modules, re-run the installer to keep skills in sync (see [Troubleshooting](#troubleshooting)).

## Skills vs. Agent Menu Triggers

Sedona offers two ways to start work, and they serve different purposes.

| Mechanism | How you invoke it | What happens |
| --- | --- | --- |
| **Skill** | Type the skill name (e.g. `sedona-help`) in your IDE | Directly loads an agent, runs a workflow, or executes a task |
...
When you run `npx sedona install`, the installer reads the manifests for every selected module and writes one skill per agent, workflow, task, and tool. Each skill is a directory containing a `SKILL.md` file that instructs the AI to load the corresponding source file and follow its instructions.
...
The directory name determines the skill name in your IDE. For example, the directory `sedona-agent-dev/` registers the skill `sedona-agent-dev`.
...
Run `sedona-help` for context-aware guidance on your next step.
...
| --- | --- | --- |
| `sedona-agent-dev` | Amelia (Developer) | Implements stories with strict adherence to specs |
| `sedona-pm` | John (Product Manager) | Creates and validates PRDs |
| `sedona-architect` | Winston (Architect) | Designs system architecture |
...
| --- | --- |
| `sedona-product-brief` | Create a product brief — guided discovery when your concept is clear |
| `sedona-prfaq` | [Working Backwards PRFAQ](../explanation/analysis-phase.md#prfaq-working-backwards) challenge to stress-test your product concept |
| `sedona-create-prd` | Create a Product Requirements Document |
| `sedona-create-architecture` | Design system architecture |
| `sedona-create-epics-and-stories` | Create epics and stories |
| `sedona-dev-story` | Implement a story |
| `sedona-code-review` | Run a code review |
| `sedona-quick-dev` | Unified quick flow — clarify intent, plan, implement, review, present |
...
**Sedona-Help: Your Intelligent Guide**

`sedona-help` is your primary interface for discovering what to do next. It inspects your project, understands natural language queries, and recommends the next required or optional step based on your installed modules.
...
All skills use the `sedona-` prefix followed by a descriptive name (e.g., `sedona-agent-dev`, `sedona-create-prd`, `sedona-help`). See [Modules](./modules.md) for available modules.
...
**Expected skills are missing.** The installer only generates skills for modules you selected. Run `npx sedona install` again and verify your module selection. Check that the skill files exist in the expected directory.
