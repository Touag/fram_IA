---
title: Core Tools
description: Reference for all built-in tasks and workflows available in every Sedona installation without additional modules.
sidebar:
  order: 2
---

Every Sedona installation includes a set of core skills that can be used in conjunction with any anything you are doing — standalone tasks and workflows that work across all projects, all modules, and all phases. These are always available regardless of which optional modules you install.

:::tip[Quick Path]
Run any core tool by typing its skill name (e.g., `sedona-help`) in your IDE. No agent session required.
:::
...
| Tool | Type | Purpose |
| --- | --- | --- |
| [`sedona-help`](#sedona-help) | Task | Get context-aware guidance on what to do next |
| [`sedona-brainstorming`](#sedona-brainstorming) | Workflow | Facilitate interactive brainstorming sessions |
| [`sedona-party-mode`](#sedona-party-mode) | Workflow | Orchestrate multi-agent group discussions |
| [`sedona-distillator`](#sedona-distillator) | Task | Lossless LLM-optimized compression of documents |
| [`sedona-advanced-elicitation`](#sedona-advanced-elicitation) | Task | Push LLM output through iterative refinement methods |
| [`sedona-review-adversarial-general`](#sedona-review-adversarial-general) | Task | Cynical review that finds what's missing and what's wrong |
| [`sedona-review-edge-case-hunter`](#sedona-review-edge-case-hunter) | Task | Exhaustive branching-path analysis for unhandled edge cases |
| [`sedona-editorial-review-prose`](#sedona-editorial-review-prose) | Task | Clinical copy-editing for communication clarity |
| [`sedona-editorial-review-structure`](#sedona-editorial-review-structure) | Task | Structural editing — cuts, merges, and reorganization |
| [`sedona-shard-doc`](#sedona-shard-doc) | Task | Split large markdown files into organized sections |
| [`sedona-index-docs`](#sedona-index-docs) | Task | Generate or update an index of all docs in a folder |
...
- You're new to Sedona and need orientation
...
**Input:** Optional query in natural language (e.g., `sedona-help I have a SaaS idea, where do I start?`)
...
**Orchestrate multi-agent group discussions.** — Loads all installed Sedona agents and facilitates a natural conversation where each agent contributes from their unique expertise and personality.
...
Run both `sedona-review-adversarial-general` and `sedona-review-edge-case-hunter` together for orthogonal coverage. The adversarial review catches quality and completeness issues; the edge case hunter catches unhandled paths.
:::
