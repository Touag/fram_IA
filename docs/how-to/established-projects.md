...
description: How to use Sedona on existing codebases
sidebar:
  order: 7
---

Use Sedona effectively when working on existing projects and legacy codebases.

This guide covers the essential workflow for onboarding to existing projects with Sedona.

:::note[Prerequisites]

- Sedona installed (`npx sedona install`)
...
If you have completed all PRD epics and stories through the Sedona process, clean up those files. Archive them, delete them, or rely on version history if needed. Do not keep these files in:
...
You can review and refine the generated file, or create it manually at `_sedona-output/project-context.md` if you prefer.
...
For complex projects, consider using the `sedona-document-project` workflow. It offers runtime variants that will scan your entire project and document its actual current state.

## Step 3: Get Help

### Sedona-Help: Your Starting Point

**Run `sedona-help` anytime you're unsure what to do next.** This intelligent guide:
...
Sedona-Help also **automatically runs at the end of every workflow**, providing clear guidance on exactly what to do next.
...
| ------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------- |
| **Small updates or additions** | Run `sedona-quick-dev` to clarify intent, plan, implement, and review in a single workflow. The full four-phase Sedona is likely overkill. |
| **Major changes or additions** | Start with the Sedona, applying as much or as little rigor as needed.                                                                    |
