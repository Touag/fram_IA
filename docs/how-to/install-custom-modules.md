...
Use the Sedona installer to add modules from the community registry, third-party Git repositories, or local file paths.

## When to Use This

- Installing a community-contributed module from the Sedona registry
- Installing a module from a third-party Git repository (GitHub, GitLab, Bitbucket, self-hosted)
- Testing a module you are developing locally with Sedona Builder
...
Community modules are curated in the [Sedona plugins marketplace](https://github.com/sedona-code-org/sedona-plugins-marketplace). They are organized by category and are pinned to an approved commit for safety.
...
If you are building a module with [Sedona Builder](https://github.com/sedona-code-org/sedona-builder), you can install it directly from your working directory:
...
If you delete the local source directory after installation, the installed module files in `_sedona/` are preserved. The module will be skipped during updates until the source path is restored.
:::

## What You Get

After installation, custom modules appear in `_sedona/` alongside official modules:
...
Use [Sedona Builder](https://github.com/sedona-code-org/sedona-builder) to create modules that others can install:

1. Run `sedona-module-builder` to scaffold your module structure
2. Add skills, agents, and workflows with the various sedona builder tools
3. Publish to a Git repository or share the folder collection
4. Others install with `--custom-source <your-repo-url>`

For modules to support discovery mode, include a `.claude-plugin/marketplace.json` in your repository root (this is a cross-tool convention, not Claude-specific). See the [Sedona Builder documentation](https://github.com/sedona-code-org/sedona-builder) for the marketplace.json format.
