...
- `sedona-create-architecture` — respects technical preferences during solutioning
- `sedona-create-story` — informs story creation with project patterns
- `sedona-dev-story` — guides implementation decisions
- `sedona-code-review` — validates against project standards
- `sedona-quick-dev` — applies patterns when implementing specs
- `sedona-sprint-planning`, `sedona-retrospective`, `sedona-correct-course` — provides project-wide context
...
|----------|----------------|---------|
| **New project, before architecture** | Manually, before `sedona-create-architecture` | Document your technical preferences so the architect respects them |
| **New project, after architecture** | Via `sedona-generate-project-context` or manually | Capture architecture decisions for implementation agents |
| **Existing project** | Via `sedona-generate-project-context` | Discover existing patterns so agents follow established conventions |
| **Quick Flow project** | Before or during `sedona-quick-dev` | Ensure quick implementation respects your patterns |
...
Create the file at `_sedona-output/project-context.md` and add your rules:
...
Run the `sedona-generate-project-context` workflow after completing your architecture:
...
For existing projects, run `sedona-generate-project-context` to discover existing patterns:
...
You can edit it manually at any time, or re-run `sedona-generate-project-context` to update it after significant changes.

:::note[File Location]
The default location is `_sedona-output/project-context.md`. Workflows search for it there, and also check `**/project-context.md` anywhere in your project.
:::
