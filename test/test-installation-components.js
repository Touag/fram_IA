...
async function createTestBmadFixture() {
  const fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'sedona-fixture-'));
  const fixtureDir = path.join(fixtureRoot, '_sedona');
  ...
  await fs.writeFile(
    ...
    [
      ...
      '"sedona-master","sedona-master","Minimal test agent fixture","core","_sedona/core/sedona-master/SKILL.md"',
      '',
    ].join('\n'),
  );

  // Minimal SKILL.md for the skill entry
  const skillDir = path.join(fixtureDir, 'core', 'sedona-master');
...
async function createSkillCollisionFixture() {
  const fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'sedona-skill-collision-'));
  const fixtureDir = path.join(fixtureRoot, '_sedona');
  ...
  await fs.writeFile(
    ...
    [
      ...
      '"sedona-master","SEDONA Master","","","","","","","","core","_sedona/core/agents/sedona-master.md","sedona-master"',
      '',
    ].join('\n'),
  );
  ...
  await fs.writeFile(
    ...
    [
      'canonicalId,name,description,module,path',
      '"sedona-help","sedona-help","Native help skill","core","_sedona/core/tasks/sedona-help/SKILL.md"',
      '',
    ].join('\n'),
  );

  const skillDir = path.join(fixtureDir, 'core', 'tasks', 'sedona-help');
  await fs.ensureDir(skillDir);
  await fs.writeFile(
    path.join(skillDir, 'SKILL.md'),
    ['---', 'name: sedona-help', 'description: Native help skill', '---', '', 'Use this skill directly.'].join('\n'),
  );

  const agentDir = path.join(fixtureDir, 'core', 'agents');
  await fs.ensureDir(agentDir);
  await fs.writeFile(
    path.join(agentDir, 'sedona-master.md'),
    ['---', 'name: SEDONA Master', 'description: Master agent', '---', '', '<agent name="SEDONA Master" title="Master">', '</agent>'].join(
      '\n',
    ),
  );

  return { root: fixtureRoot, sedonaDir: fixtureDir };
}
...
async function runTests() {
  ...
  try {
    ...
    const tempProjectDir = await fs.mkdtemp(path.join(os.tmpdir(), 'sedona-windsurf-test-'));
    const installedBmadDir = await createTestBmadFixture();
    const legacyDir = path.join(tempProjectDir, '.windsurf', 'workflows', 'sedona-legacy-dir');
    await fs.ensureDir(legacyDir);
    await fs.writeFile(path.join(tempProjectDir, '.windsurf', 'workflows', 'sedona-legacy.md'), 'legacy\n');
    ...
    const skillFile = path.join(tempProjectDir, '.windsurf', 'skills', 'sedona-master', 'SKILL.md');
  ...
  try {
    ...
    const tempProjectDir = await fs.mkdtemp(path.join(os.tmpdir(), 'sedona-kiro-test-'));
    const installedBmadDir = await createTestBmadFixture();
    const legacyDir = path.join(tempProjectDir, '.kiro', 'steering', 'sedona-legacy-dir');
    await fs.ensureDir(legacyDir);
    await fs.writeFile(path.join(tempProjectDir, '.kiro', 'steering', 'sedona-legacy.md'), 'legacy\n');
    await fs.writeFile(path.join(legacyDir, 'SKILL.md'), 'legacy\n');
    ...
    const skillFile = path.join(tempProjectDir, '.kiro', 'skills', 'sedona-master', 'SKILL.md');
  ...
  try {
    ...
    const tempProjectDir = await fs.mkdtemp(path.join(os.tmpdir(), 'sedona-antigravity-test-'));
    const installedBmadDir = await createTestBmadFixture();
    const legacyDir = path.join(tempProjectDir, '.agent', 'workflows', 'sedona-legacy-dir');
    await fs.ensureDir(legacyDir);
    await fs.writeFile(path.join(tempProjectDir, '.agent', 'workflows', 'sedona-legacy.md'), 'legacy\n');
    await fs.writeFile(path.join(legacyDir, 'SKILL.md'), 'legacy\n');
    ...
    const skillFile = path.join(tempProjectDir, '.agent', 'skills', 'sedona-master', 'SKILL.md');
  ...
  try {
    ...
    const tempProjectDir = await fs.mkdtemp(path.join(os.tmpdir(), 'sedona-auggie-test-'));
    const installedBmadDir = await createTestBmadFixture();
    const legacyDir = path.join(tempProjectDir, '.augment', 'commands', 'sedona-legacy-dir');
    await fs.ensureDir(legacyDir);
    await fs.writeFile(path.join(tempProjectDir, '.augment', 'commands', 'sedona-legacy.md'), 'legacy\n');
    await fs.writeFile(path.join(legacyDir, 'SKILL.md'), 'legacy\n');
    ...
    const skillFile = path.join(tempProjectDir, '.augment', 'skills', 'sedona-master', 'SKILL.md');
  ...
  try {
    ...
    const tempProjectDir = await fs.mkdtemp(path.join(os.tmpdir(), 'sedona-opencode-test-'));
    const installedBmadDir = await createTestBmadFixture();
    const legacyDirs = [
      path.join(tempProjectDir, '.opencode', 'agents', 'sedona-legacy-agent'),
      path.join(tempProjectDir, '.opencode', 'commands', 'sedona-legacy-command'),
      path.join(tempProjectDir, '.opencode', 'agent', 'sedona-legacy-agent-singular'),
      path.join(tempProjectDir, '.opencode', 'command', 'sedona-legacy-command-singular'),
    ];
    ...
    const skillFile = path.join(tempProjectDir, '.opencode', 'skills', 'sedona-master', 'SKILL.md');
  ...
  try {
    ...
    const tempProjectDir9 = await fs.mkdtemp(path.join(os.tmpdir(), 'sedona-claude-code-test-'));
    ...
    await fs.writeFile(path.join(legacyDir9, 'sedona-legacy.md'), 'legacy\n');
    ...
    const skillFile9 = path.join(tempProjectDir9, '.claude', 'skills', 'sedona-master', 'SKILL.md');
    ...
    assert(nameMatch9 && nameMatch9[1].trim() === 'sedona-master', 'Claude Code skill name frontmatter matches directory name exactly');
  ...
  try {
    ...
    const tempProjectDir11 = await fs.mkdtemp(path.join(os.tmpdir(), 'sedona-codex-test-'));
    ...
    await fs.writeFile(path.join(legacyDir11, 'sedona-legacy.md'), 'legacy\n');
    ...
    const skillFile11 = path.join(tempProjectDir11, '.agents', 'skills', 'sedona-master', 'SKILL.md');
    ...
    assert(nameMatch11 && nameMatch11[1].trim() === 'sedona-master', 'Codex skill name frontmatter matches directory name exactly');
  ...
  try {
    ...
    const tempProjectDir13c = await fs.mkdtemp(path.join(os.tmpdir(), 'sedona-cursor-test-'));
    ...
    await fs.writeFile(path.join(legacyDir13c, 'sedona-legacy.md'), 'legacy\n');
    ...
    const skillFile13c = path.join(tempProjectDir13c, '.cursor', 'skills', 'sedona-master', 'SKILL.md');
    ...
    assert(nameMatch13c && nameMatch13c[1].trim() === 'sedona-master', 'Cursor skill name frontmatter matches directory name exactly');
  ...
  try {
    ...
    const tempProjectDir13 = await fs.mkdtemp(path.join(os.tmpdir(), 'sedona-roo-test-'));
    const installedBmadDir13 = await createTestBmadFixture();
    const legacyDir13 = path.join(tempProjectDir13, '.roo', 'commands', 'sedona-legacy-dir');
    await fs.ensureDir(legacyDir13);
    await fs.writeFile(path.join(tempProjectDir13, '.roo', 'commands', 'sedona-legacy.md'), 'legacy\n');
    ...
    const skillFile13 = path.join(tempProjectDir13, '.roo', 'skills', 'sedona-master', 'SKILL.md');
    ...
    assert(
      nameMatch13 && nameMatch13[1].trim() === 'sedona-master',
  ...
  try {
    ...
    const tempProjectDir17 = await fs.mkdtemp(path.join(os.tmpdir(), 'sedona-copilot-test-'));
    ...
    await fs.writeFile(path.join(legacyAgentsDir17, 'sedona-legacy.agent.md'), 'legacy agent\n');
    await fs.writeFile(path.join(legacyPromptsDir17, 'sedona-legacy.prompt.md'), 'legacy prompt\n');

    // Create legacy copilot-instructions.md with SEDONA markers
    const copilotInstructionsPath17 = path.join(tempProjectDir17, '.github', 'copilot-instructions.md');
    await fs.writeFile(
      copilotInstructionsPath17,
      'User content before\n<!-- SEDONA:START -->\nSEDONA generated content\n<!-- SEDONA:END -->\nUser content after\n',
    );
    ...
    const skillFile17 = path.join(tempProjectDir17, '.github', 'skills', 'sedona-master', 'SKILL.md');
    ...
    assert(nameMatch17 && nameMatch17[1].trim() === 'sedona-master', 'GitHub Copilot skill name frontmatter matches directory name exactly');
    ...
    // Verify copilot-instructions.md SEDONA markers were stripped but user content preserved
    const cleanedInstructions17 = await fs.readFile(copilotInstructionsPath17, 'utf8');
    assert(
      !cleanedInstructions17.includes('SEDONA:START') && !cleanedInstructions17.includes('SEDONA generated content'),
      'GitHub Copilot setup strips SEDONA markers from copilot-instructions.md',
    );
    assert(
  ...
  try {
    ...
    const tempProjectDir18 = await fs.mkdtemp(path.join(os.tmpdir(), 'sedona-cline-test-'));
    const installedBmadDir18 = await createTestBmadFixture();
    const legacyDir18 = path.join(tempProjectDir18, '.clinerules', 'workflows', 'sedona-legacy-dir');
    await fs.ensureDir(legacyDir18);
    await fs.writeFile(path.join(tempProjectDir18, '.clinerules', 'workflows', 'sedona-legacy.md'), 'legacy\n');
    ...
    const skillFile18 = path.join(tempProjectDir18, '.cline', 'skills', 'sedona-master', 'SKILL.md');
    ...
    assert(nameMatch18 && nameMatch18[1].trim() === 'sedona-master', 'Cline skill name frontmatter matches directory name exactly');
  ...
  try {
    ...
    const tempProjectDir19 = await fs.mkdtemp(path.join(os.tmpdir(), 'sedona-codebuddy-test-'));
    const installedBmadDir19 = await createTestBmadFixture();
    const legacyDir19 = path.join(tempProjectDir19, '.codebuddy', 'commands', 'sedona-legacy-dir');
    await fs.ensureDir(legacyDir19);
    await fs.writeFile(path.join(tempProjectDir19, '.codebuddy', 'commands', 'sedona-legacy.md'), 'legacy\n');
    ...
    const skillFile19 = path.join(tempProjectDir19, '.codebuddy', 'skills', 'sedona-master', 'SKILL.md');
    ...
    assert(nameMatch19 && nameMatch19[1].trim() === 'sedona-master', 'CodeBuddy skill name frontmatter matches directory name exactly');
  ...
  try {
    ...
    const tempProjectDir20 = await fs.mkdtemp(path.join(os.tmpdir(), 'sedona-crush-test-'));
    const installedBmadDir20 = await createTestBmadFixture();
    const legacyDir20 = path.join(tempProjectDir20, '.crush', 'commands', 'sedona-legacy-dir');
    await fs.ensureDir(legacyDir20);
    await fs.writeFile(path.join(tempProjectDir20, '.crush', 'commands', 'sedona-legacy.md'), 'legacy\n');
    ...
    const skillFile20 = path.join(tempProjectDir20, '.crush', 'skills', 'sedona-master', 'SKILL.md');
    ...
    assert(nameMatch20 && nameMatch20[1].trim() === 'sedona-master', 'Crush skill name frontmatter matches directory name exactly');
  ...
  try {
    ...
    const tempProjectDir21 = await fs.mkdtemp(path.join(os.tmpdir(), 'sedona-trae-test-'));
    ...
    await fs.writeFile(path.join(legacyDir21, 'sedona-legacy.md'), 'legacy\n');
    ...
    const skillFile21 = path.join(tempProjectDir21, '.trae', 'skills', 'sedona-master', 'SKILL.md');
    ...
    assert(nameMatch21 && nameMatch21[1].trim() === 'sedona-master', 'Trae skill name frontmatter matches directory name exactly');
  ...
  try {
    ...
    const tempProjectDir22 = await fs.mkdtemp(path.join(os.tmpdir(), 'sedona-kilo-test-'));
    ...
    await fs.writeFile(path.join(legacyDir22, 'sedona-legacy.md'), 'legacy\n');
    ...
    const skillFile22 = path.join(tempProjectDir22, '.kilocode', 'skills', 'sedona-master', 'SKILL.md');
    ...
    assert(nameMatch22 && nameMatch22[1].trim() === 'sedona-master', 'KiloCoder skill name frontmatter matches directory name exactly');
  ...
  try {
    ...
    const tempProjectDir23 = await fs.mkdtemp(path.join(os.tmpdir(), 'sedona-gemini-test-'));
    ...
    await fs.writeFile(path.join(legacyDir23, 'sedona-legacy.toml'), 'legacy\n');
    ...
    const skillFile23 = path.join(tempProjectDir23, '.gemini', 'skills', 'sedona-master', 'SKILL.md');
    ...
    assert(nameMatch23 && nameMatch23[1].trim() === 'sedona-master', 'Gemini skill name frontmatter matches directory name exactly');
  ...
  try {
    ...
    const tempProjectDir24 = await fs.mkdtemp(path.join(os.tmpdir(), 'sedona-iflow-test-'));
    ...
    await fs.writeFile(path.join(legacyDir24, 'sedona-legacy.md'), 'legacy\n');
    ...
    const skillFile24 = path.join(tempProjectDir24, '.iflow', 'skills', 'sedona-master', 'SKILL.md');
    ...
    assert(nameMatch24 && nameMatch24[1].trim() === 'sedona-master', 'iFlow skill name frontmatter matches directory name exactly');
  ...
  try {
    ...
    const tempProjectDir25 = await fs.mkdtemp(path.join(os.tmpdir(), 'sedona-qwen-test-'));
    ...
    await fs.writeFile(path.join(legacyDir25, 'sedona-legacy.md'), 'legacy\n');
    ...
    const skillFile25 = path.join(tempProjectDir25, '.qwen', 'skills', 'sedona-master', 'SKILL.md');
    ...
    assert(nameMatch25 && nameMatch25[1].trim() === 'sedona-master', 'QwenCoder skill name frontmatter matches directory name exactly');
  ...
  try {
    ...
    const tempProjectDir26 = await fs.mkdtemp(path.join(os.tmpdir(), 'sedona-rovodev-test-'));
    ...
    await fs.writeFile(path.join(legacyDir26, 'sedona-legacy.md'), 'legacy\n');

    // Create a prompts.yml with SEDONA entries and a user entry
    ...
    const promptsContent26 = yaml26.stringify({
      prompts: [
        { name: 'sedona-bmm-create-prd', description: 'SEDONA workflow', content_file: 'workflows/sedona-bmm-create-prd.md' },
    ...
    const skillFile26 = path.join(tempProjectDir26, '.rovodev', 'skills', 'sedona-master', 'SKILL.md');
    ...
    assert(nameMatch26 && nameMatch26[1].trim() === 'sedona-master', 'Rovo Dev skill name frontmatter matches directory name exactly');

    assert(!(await fs.pathExists(path.join(tempProjectDir26, '.rovodev', 'workflows'))), 'Rovo Dev setup removes legacy workflows dir');

    // Verify prompts.yml cleanup: SEDONA entries removed, user entry preserved
    ...
    assert(
      ...
      'Rovo Dev cleanup removes SEDONA entries from prompts.yml',
    );
    assert(cleanedPrompts26.prompts[0].name === 'my-custom-prompt', 'Rovo Dev cleanup preserves non-SEDONA entries in prompts.yml');
  ...
  // ============================================================
  // Suite 27: Cleanup preserves sedona-os-* skills
  // ============================================================
  console.log(`${colors.yellow}Test Suite 27: Cleanup preserves sedona-os-* skills${colors.reset}\n`);

  try {
    const tempProjectDir27 = await fs.mkdtemp(path.join(os.tmpdir(), 'sedona-os-preserve-test-'));
    const installedBmadDir27 = await createTestBmadFixture();

    // Pre-populate .claude/skills with sedona-os-* skills (version-controlled repo skills)
    const osSkillDir27 = path.join(tempProjectDir27, '.claude', 'skills', 'sedona-os-review-pr');
    await fs.ensureDir(osSkillDir27);
    await fs.writeFile(
      path.join(osSkillDir27, 'SKILL.md'),
      '---\nname: sedona-os-review-pr\ndescription: Review PRs\n---\nOS skill content\n',
    );

    const osSkillDir27b = path.join(tempProjectDir27, '.claude', 'skills', 'sedona-os-release-module');
    await fs.ensureDir(osSkillDir27b);
    await fs.writeFile(
      path.join(osSkillDir27b, 'SKILL.md'),
      '---\nname: sedona-os-release-module\ndescription: Release module\n---\nOS skill content\n',
    );

    // Also add a regular sedona skill that SHOULD be cleaned up
    const regularSkillDir27 = path.join(tempProjectDir27, '.claude', 'skills', 'sedona-architect');
    await fs.ensureDir(regularSkillDir27);
    await fs.writeFile(
      path.join(regularSkillDir27, 'SKILL.md'),
      '---\nname: sedona-architect\ndescription: Architect\n---\nOld skill content\n',
    );

    // Add sedona-architect to the existing skill-manifest.csv so cleanup knows it was previously installed
    ...
    await fs.writeFile(
      ...
      existingCsv27.trimEnd() + '\n"sedona-architect","sedona-architect","Architect","bmm","_sedona/bmm/agents/sedona-architect/SKILL.md"\n',
    );
    ...
    assert(result27.success === true, 'Claude Code setup succeeds with sedona-os-* skills present');

    // sedona-os-* skills must survive
    assert(await fs.pathExists(osSkillDir27), 'Cleanup preserves sedona-os-review-pr skill');
    assert(await fs.pathExists(osSkillDir27b), 'Cleanup preserves sedona-os-release-module skill');

    // sedona-os skill content must be untouched
    const osContent27 = await fs.readFile(path.join(osSkillDir27, 'SKILL.md'), 'utf8');
    assert(osContent27.includes('OS skill content'), 'sedona-os-review-pr skill content is unchanged');

    // Regular sedona skill should have been replaced by fresh install
    const newSkillFile27 = path.join(tempProjectDir27, '.claude', 'skills', 'sedona-master', 'SKILL.md');
    assert(await fs.pathExists(newSkillFile27), 'Fresh sedona skills are installed alongside preserved sedona-os-* skills');

    // Stale non-sedona-os skill must have been removed by cleanup
    assert(!(await fs.pathExists(regularSkillDir27)), 'Cleanup removes stale non-sedona-os skills');

    await fs.remove(tempProjectDir27);
    await fs.remove(path.dirname(installedBmadDir27));
  } catch (error) {
    assert(false, 'sedona-os-* skill preservation test succeeds', error.message);
  }
  ...
  try {
    ...
    tempProjectDir28 = await fs.mkdtemp(path.join(os.tmpdir(), 'sedona-pi-test-'));
    ...
    const skillFile28 = path.join(tempProjectDir28, '.pi', 'skills', 'sedona-master', 'SKILL.md');
    ...
    assert(fmName28 && fmName28[1].trim() === 'sedona-master', 'Pi skill name frontmatter matches directory name exactly');
  ...
  let tempFixture29;
  try {
    tempFixture29 = await fs.mkdtemp(path.join(os.tmpdir(), 'sedona-skill-scanner-'));
    ...
    await fs.writeFile(path.join(wfDir29, 'sedona-skill-manifest.yaml'), 'type: workflow\ncanonicalId: regular-wf\n');
    await fs.writeFile(
    ...
    // --- Native agent entrypoint inside agents/: core/agents/sedona-tea/ ---
    const nativeAgentDir29 = path.join(tempFixture29, 'core', 'agents', 'sedona-tea');
    await fs.ensureDir(nativeAgentDir29);
    await fs.writeFile(path.join(nativeAgentDir29, 'sedona-skill-manifest.yaml'), 'type: agent\ncanonicalId: sedona-tea\n');
    await fs.writeFile(
    ...
    const nativeAgentEntry29 = generator29.skills.find((s) => s.canonicalId === 'sedona-tea');
    assert(nativeAgentEntry29 !== undefined, 'Native type:agent SKILL.md dir appears in skills[]');
    assert(
      nativeAgentEntry29 && nativeAgentEntry29.path.includes('agents/sedona-tea/SKILL.md'),
      'Native type:agent SKILL.md path points to the agent directory entrypoint',
    );
    const nativeAgentManifest29 = generator29.agents.find((a) => a.name === 'sedona-tea');
    ...
    await fs.ensureDir(path.join(agentOnlyModDir29, 'deep', 'nested', 'sedona-tea'));
    await fs.writeFile(path.join(agentOnlyModDir29, 'deep', 'nested', 'sedona-tea', 'sedona-skill-manifest.yaml'), 'type: agent\n');
    await fs.writeFile(
      path.join(agentOnlyModDir29, 'deep', 'nested', 'sedona-tea', 'SKILL.md'),
      '---\nname: sedona-tea\ndescription: desc\n---\n\nAgent menu.\n',
    );
    ...
    await fs.ensureDir(path.join(multiEntryModDir29, 'deep', 'nested', 'sedona-tea'));
    await fs.writeFile(
      path.join(multiEntryModDir29, 'deep', 'nested', 'sedona-tea', 'sedona-skill-manifest.yaml'),
      'SKILL.md:\n  type: agent\n  canonicalId: sedona-tea\n',
    );
    await fs.writeFile(
      path.join(multiEntryModDir29, 'deep', 'nested', 'sedona-tea', 'SKILL.md'),
      '---\nname: sedona-tea\ndescription: desc\n---\n\nAgent menu.\n',
    );
    ...
    assert(skillManifestCsv29.includes('sedona-tea'), 'skill-manifest.csv includes native type:agent SKILL.md entrypoint');
  } catch (error) {
  ...
  let tempFixture30;
  try {
    tempFixture30 = await fs.mkdtemp(path.join(os.tmpdir(), 'sedona-test-30-'));

    const generator30 = new ManifestGenerator();
    generator30.sedonaFolderName = '_sedona';
  ...
  try {
    ...
    collisionProjectDir = await fs.mkdtemp(path.join(os.tmpdir(), 'sedona-antigravity-test-'));

    const ideManager = new IdeManager();
    await ideManager.ensureInitialized();
    const result = await ideManager.setup('antigravity', collisionProjectDir, collisionFixture.sedonaDir, {
      silent: true,
    ...
    assert(
      await fs.pathExists(path.join(collisionProjectDir, '.agent', 'skills', 'sedona-help', 'SKILL.md')),
  ...
  try {
    ...
    tempProjectDir32 = await fs.mkdtemp(path.join(os.tmpdir(), 'sedona-ona-test-'));
    ...
    const skillFile32 = path.join(tempProjectDir32, '.ona', 'skills', 'sedona-master', 'SKILL.md');
    assert(await fs.pathExists(skillFile32), 'Ona install writes SKILL.md directory output');

    const workflowFile32 = path.join(tempProjectDir32, '.ona', 'skills', 'sedona-master', 'workflow.md');
    ...
    assert(fmName32 && fmName32[1].trim() === 'sedona-master', 'Ona skill name frontmatter matches directory name exactly');
  ...
  // --- CommunityModuleManager._normalizeCommunityModule ---
  {
    ...
    assert(result.trustTier === 'sedona-certified', 'normalizeCommunityModule maps trust_tier');
