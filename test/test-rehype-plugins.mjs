...
function runTests() {
  ...
  {
    const tree = makeAnchorTree('page.md');
    transform(tree, STD_FILE, { ...STD_OPTS, base: '/SEDONA/' });
    assert(getHref(tree) === '/SEDONA/guide/page/', 'Base /SEDONA/ -> /SEDONA/guide/page/', `Got ${getHref(tree)}`);
  }

  {
    const tree = makeAnchorTree('page.md');
    transform(tree, STD_FILE, { ...STD_OPTS, base: '/SEDONA' });
    assert(getHref(tree) === '/SEDONA/guide/page/', 'Base /SEDONA (no trailing slash) -> same result', `Got ${getHref(tree)}`);
  }
  ...
  {
    const tree = makeAnchorTree('/page/');
    transformBase(tree, { base: '/SEDONA/' });
    assert(getHref(tree) === '/SEDONA/page/', 'Base /SEDONA/ prefixes', `Got ${getHref(tree)}`);
  }

  {
    const tree = makeAnchorTree('/page/');
    transformBase(tree, { base: '/SEDONA' });
    assert(getHref(tree) === '/SEDONA/page/', 'Base /SEDONA normalizes (adds trailing slash)', `Got ${getHref(tree)}`);
  }
...

  {
    ...
    assert(getHref(tree) === '/SEDONA/page/', 'a[href] prefixed', `Got ${getHref(tree)}`);
  }

  {
    const tree = makeElementTree('img', { src: '/img/logo.png' });
    transformBase(tree, { base: BASE });
    assert(getSrc(tree) === '/SEDONA/img/logo.png', 'img[src] prefixed', `Got ${getSrc(tree)}`);
  }

  {
    ...
    transformBase(tree, { base: BASE });
    assert(
      tree.children[0].properties.href === '/SEDONA/styles/main.css',
      'link[href] prefixed',
      `Got ${tree.children[0].properties.href}`,
    );
  }
...

  {
    ...
    transformBase(tree, { base: BASE });
    assert(getSrc(tree) === '/SEDONA/media/intro.mp4', 'video[src] prefixed', `Got ${getSrc(tree)}`);
  }

  {
    const tree = makeElementTree('audio', { src: '/media/clip.mp3' });
    transformBase(tree, { base: BASE });
    assert(getSrc(tree) === '/SEDONA/media/clip.mp3', 'audio[src] prefixed', `Got ${getSrc(tree)}`);
  }

  {
    const tree = makeElementTree('iframe', { src: '/embed/widget' });
    transformBase(tree, { base: BASE });
    assert(getSrc(tree) === '/SEDONA/embed/widget', 'iframe[src] prefixed', `Got ${getSrc(tree)}`);
  }
...

  {
    ...
    transformBase(tree, { base: BASE });
    assert(getSrc(tree) === '/SEDONA/media/alt.mp4', 'source[src] prefixed', `Got ${getSrc(tree)}`);
  }
  ...
  {
    const tree = makeAnchorTree('/SEDONA/page/');
    transformBase(tree, { base: BASE });
    assert(getHref(tree) === '/SEDONA/page/', 'Already prefixed skipped', `Got ${getHref(tree)}`);
  }
...

  {
    ...
    transformBase(tree, { base: BASE });
    assert(getRawValue(tree) === '<img src="/SEDONA/img/logo.png">', 'Raw img src rewritten', `Got ${getRawValue(tree)}`);
  }

  {
    ...
    transformBase(tree, { base: BASE });
    assert(getRawValue(tree) === '<a href="/SEDONA/page/">link</a>', 'Raw a href rewritten', `Got ${getRawValue(tree)}`);
  }
...

  {
    const tree = {
      ...
      children: [
        {
          type: 'raw',
          value: '<img src="/SEDONA/img/logo.png">',
        },
      ],
    };
    transformBase(tree, { base: BASE });
    assert(getRawValue(tree) === '<img src="/SEDONA/img/logo.png">', 'Raw already prefixed unchanged', `Got ${getRawValue(tree)}`);
  }

  {
    ...
    transformBase(tree, { base: BASE });
    assert(
      getRawValue(tree) === '<a href="/SEDONA/page/"><img src="/SEDONA/img/logo.png"></a>',
...

  {
    ...
    transformBase(tree, { base: BASE });
    const href = getHref(tree);
    assert(href === '/SEDONA/guide/sibling/', './sibling.md through both -> no double prefix', `Got ${href}`);
  }

  {
    ...
    transformBase(tree, { base: BASE });
    assert(getSrc(tree) === '/SEDONA/img/logo.png', 'img /img/logo.png -> only base-paths prefixes', `Got ${getSrc(tree)}`);
  }
...

  {
    ...
    transform(tree, STD_FILE, { ...STD_OPTS, base: BASE });
    transformBase(tree, { base: BASE });
    assert(getHref(tree) === '/SEDONA/page/', '/page/ (non-.md) -> only base-paths prefixes', `Got ${getHref(tree)}`);
  }
