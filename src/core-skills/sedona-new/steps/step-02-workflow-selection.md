...
**[A] Jira** — Configurer Jira immédiatement. Follow this sequence:

#### A1. Afficher les instructions de préparation

```
**🚀 Lysandre:** Parfait — on configure la sync Jira maintenant.

Avant de commencer, assure-toi d'avoir :

**1. Un projet Jira dédié à {project_name}**
   → Crée-le sur ton espace Atlassian si ce n'est pas déjà fait.
   ⚠️  N'utilise PAS un projet existant avec d'autres tickets — le sync va créer des tickets en masse.

**2. Ton API token Atlassian**
   → Génère-le ici : https://id.atlassian.com/manage-profile/security/api-tokens
   → Nomme-le "sedona-{project_name}" — il ne sera visible qu'une fois, garde-le.

Tu as tout ça ? [O]ui / [A]ttends, je vais le créer
```

**HALT — wait for user.**

If user says wait: tell them to come back when ready and skip Jira config for now (save `pm_mode: jira_pending`).

#### A2. Collecter les credentials Jira

```
**🚀 Lysandre:** Donne-moi ces 4 infos :
...
**[3] API Token**
(celui que tu viens de générer)

**[4] Clé du projet Jira**
Ex: si tes tickets sont SCRUM-1, SCRUM-2 → la clé est SCRUM
```

**HALT — wait for user.**

#### A3. Valider la connexion
...
- HTTP 200 → OK
- HTTP 401 → mauvais email ou token
- HTTP 403 → token valide mais droits insuffisants

```
**🚀 Lysandre:** Connexion Jira [✓ OK / ✗ Échec — {raison}]
```

If failed → ask to check credentials and retry. Do NOT proceed until connection is confirmed.

#### A4. Écrire la config Jira

Create `{project-root}/_sedona/core/jira-config.yaml`:
```yaml
# Jira Configuration — generated by sedona-new
# WARNING: Ce fichier contient ton API token. Ne le commite JAMAIS.
...
Save `pm_mode: jira` in project-setup.md.

```
**🚀 Lysandre:** ✓ Config Jira sauvegardée dans `_sedona/core/jira-config.yaml` (gitignored).
Quand tu lances `sedona-jira-sync`, Raphaël utilisera cette config automatiquement — rien à reconfigurer.
```
