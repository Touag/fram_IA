...
/**
 * Find SEDONA directory in project
 */
function findBmadDir(projectDir = process.cwd()) {
  const possibleNames = ['_sedona'];

  for (const name of possibleNames) {
    const sedonaDir = path.join(projectDir, name);
    if (fs.existsSync(sedonaDir)) {
      return sedonaDir;
    }
  }
...
async function migrate(directory) {
  ...
  const sedonaDir = findBmadDir(projectRoot);

  if (!sedonaDir) {
    console.error(chalk.red('✗ No SEDONA installation found in directory'));
    process.exit(1);
  }

  console.log(chalk.blue.bold('🔄 SEDONA Custom Module Path Migration'));
  console.log(chalk.dim(`Project: ${projectRoot}`));
  console.log(chalk.dim(`SEDONA Directory: ${sedonaDir}`));
