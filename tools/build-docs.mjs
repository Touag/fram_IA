/**
 * SEDONA Documentation Build Pipeline
 *
...
const REPO_URL = 'https://github.com/sedona-code-org/SEDONA';
...
// Main Entry Point
/**
 * Orchestrates the full SEDONA documentation build pipeline.
 *
...
async function main() {
  ...
  console.log();
  printBanner('SEDONA Documentation Build Pipeline');
  console.log();
...
function generateLlmsTxt(outputDir) {
  ...
  const content = [
    ...
    '## Quick Start',
    '',
    `- **[Getting Started](${siteUrl}/tutorials/getting-started/)** - Tutorial: install and learn how Sedona works`,
    `- **[Installation](${siteUrl}/how-to/install-sedona/)** - How to install Sedona`,
    '',
    '## Core Concepts',
    '',
...
function generateLlmsFullTxt(docsDir, outputDir) {
  ...
  const output = [
    '# SEDONA Method Documentation (Full)',
    '',
