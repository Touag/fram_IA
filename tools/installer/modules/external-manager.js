...
const REGISTRY_RAW_URL = 'https://raw.githubusercontent.com/sedona-code-org/sedona-plugins-marketplace/main/registry/official.yaml';
const FALLBACK_CONFIG_PATH = path.join(__dirname, 'registry-fallback.yaml');

/**
 * Manages official modules from the remote Sedona marketplace registry.
...
class ExternalModuleManager {
  ...
  async loadExternalModulesConfig() {
    ...
    try {
      ...
      const config = yaml.parse(content);
      this.cachedModules = config;
      await prompts.log.warn('Could not reach Sedona registry; using bundled module list.');
      return config;
    } catch (error) {
   ...
   */
  getExternalCacheDir() {
    const cacheDir = path.join(os.homedir(), '.sedona', 'cache', 'external-modules');
    return cacheDir;
  }
  ...
  async cloneExternalModule(moduleCode, options = {}) {
    ...
    if (!moduleInfo) {
      throw new Error(`External module '${moduleCode}' not found in the Sedona registry`);
    }
