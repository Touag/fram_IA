...
class OfficialModules {
   ...
   * @param {string} sedonaDir - Target sedona directory
   ...
   */
  async install(moduleName, sedonaDir, fileTrackingCallback = null, options = {}) {
    // Check if this module has a plugin resolution (custom marketplace install)
    const { CustomModuleManager } = require('./custom-module-manager');
    const customMgr = new CustomModuleManager();
    const resolved = customMgr.getResolution(moduleName);
    if (resolved) {
      return this.installFromResolution(resolved, sedonaDir, fileTrackingCallback, options);
    }

    const sourcePath = await this.findModuleSource(moduleName, { silent: options.silent });
    const targetPath = path.join(sedonaDir, moduleName);
    ...
    if (!options.skipModuleInstaller) {
      await this.createModuleDirectories(moduleName, sedonaDir, options);
    }

    const { Manifest } = require('../core/manifest');
    const manifestObj = new Manifest();
    const versionInfo = await manifestObj.getModuleVersionInfo(moduleName, sedonaDir, sourcePath);

    await manifestObj.addModule(sedonaDir, moduleName, {
   ...
   */
  async installFromResolution(resolved, sedonaDir, fileTrackingCallback = null, options = {}) {
    const targetPath = path.join(sedonaDir, resolved.code);
    ...
    if (!options.skipModuleInstaller) {
      await this.createModuleDirectories(resolved.code, sedonaDir, options);
    }
    ...
    await manifestObj.addModule(sedonaDir, resolved.code, {
   ...
   * @param {string} sedonaDir - Target sedona directory
   */
  async update(moduleName, sedonaDir) {
   ...
   * @param {string} sedonaDir - Target sedona directory
   */
  async remove(moduleName, sedonaDir) {
    const targetPath = path.join(sedonaDir, moduleName);
   ...
   */
  async isInstalled(moduleName, sedonaDir) {
    const targetPath = path.join(sedonaDir, moduleName);
   ...
   */
  async getInstalledInfo(moduleName, sedonaDir) {
    const targetPath = path.join(sedonaDir, moduleName);
   ...
   */
  async createModuleDirectories(moduleName, sedonaDir, options = {}) {
   ...
   * @returns {Promise<string>} Path to sedona directory
   */
  async findBmadDir(projectDir) {
    // Check if project directory exists
    if (!(await fs.pathExists(projectDir))) {
      // Project doesn't exist yet, return default
      return path.join(projectDir, 'sedona');
    }
  ...
  /**
   * Detect the existing SEDONA folder name in a project
   * @param {string} projectDir - Project directory
  ...
  async loadExistingConfig(projectDir) {
    ...
    // Find the actual sedona directory (handles custom folder names)
    ...
    // Dynamically discover all installed modules by scanning sedona directory
    ...
    for (const entry of entries) {
      if (entry.isDirectory()) {
        ...
        const moduleConfigPath = path.join(sedonaDir, entry.name, 'config.yaml');
   ...
   * For example, "{test_artifacts}/{value}" + "_sedona-output/test-artifacts/test-design"
