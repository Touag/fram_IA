...
class CustomModuleManager {
   ...
   */
  getCacheDir() {
    return path.join(os.homedir(), '.sedona', 'cache', 'custom-modules');
  }
  ...
  async cloneRepo(sourceInput, options = {}) {
    ...
    const metadataPath = path.join(repoCacheDir, '.sedona-source.json');
   ...
   * @param {string} moduleCode - Module code to search for
   * @param {Object} [options] - Options (must include sedonaDir or will search common locations)
  ...
  async _findLocalSourceFromManifest(moduleCode, options = {}) {
    try {
      ...
      // Try to find sedonaDir from options or common locations
      const sedonaDir = options.sedonaDir;
      if (!sedonaDir) return null;

      const manifestData = await manifestObj.read(sedonaDir);
   ...
   * A repo root is identified by containing .sedona-source.json (new) or .claude-plugin/ (legacy).
  ...
  async _findCacheRepoRoots(dir, depth = 0, maxDepth = 4) {
    ...
    const metadataPath = path.join(dir, '.sedona-source.json');
