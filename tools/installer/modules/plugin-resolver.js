...
class PluginResolver {
   ...
   * Strips common prefixes like "sedona-" or "sedona-agent-".
   * @param {string} name - Raw name
   * @returns {string} Formatted display name
   */
  _formatDisplayName(name) {
    let cleaned = name.replace(/^sedona-agent-/, '').replace(/^sedona-/, '');
    return cleaned
      .split(/[-_]/)
  ...
  _generateMenuCode(name) {
    const cleaned = name.replace(/^sedona-agent-/, '').replace(/^sedona-/, '');
