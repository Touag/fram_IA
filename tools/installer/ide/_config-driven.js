...
const { SEDONA_FOLDER_NAME } = require('./shared/path-utils');

/**
 * Config-driven IDE setup handler
 *
 * This class provides a standardized way to install SEDONA artifacts to IDEs
...
class ConfigDrivenIdeSetup {
  ...
  setBmadFolderName(sedonaFolderName) {
    this.sedonaFolderName = sedonaFolderName;
  }

  /**
   * Detect whether this IDE already has configuration in the project.
   * Checks for sedona-prefixed entries in target_dir.
  ...
  async detect(projectDir) {
    ...
    if (await fs.pathExists(dir)) {
      try {
        ...
        return entries.some((e) => typeof e === 'string' && e.startsWith('sedona'));
      } catch {
        return false;
      }
    }
   ...
   */
  async setup(projectDir, sedonaDir, options = {}) {
    // Check for SEDONA files in ancestor directories that would cause duplicates
    if (this.installerConfig?.ancestor_conflict_check) {
      const conflict = await this.findAncestorConflict(projectDir);
      if (conflict) {
        await prompts.log.error(
          `Found existing SEDONA skills in ancestor installation: ${conflict}\n` +
            `  ${this.name} inherits skills from parent directories, so this would cause duplicates.\n` +
            `  Please remove the SEDONA files from that directory first:\n` +
            `    rm -rf "${conflict}"/sedona*`,
        );
        return {
          success: false,
    ...
    // Clean up any old SEDONA installation first
    await this.cleanup(projectDir, options, sedonaDir);
    ...
    if (this.installerConfig.target_dir) {
      return this.installToTarget(projectDir, sedonaDir, this.installerConfig, options);
    }
   ...
   */
  async installToTarget(projectDir, sedonaDir, config, options) {
    ...
    results.skills = await this.installVerbatimSkills(projectDir, sedonaDir, targetPath, config);
   ...
   */
  async installVerbatimSkills(projectDir, sedonaDir, targetPath, config) {
    ...
    for (const record of records) {
      ...
      // path is like "_sedona/bmm/workflows/sedona-quick-flow/sedona-quick-dev-new-preview/SKILL.md"
      // Strip sedonaFolderName prefix and join with sedonaDir, then get dirname
      const relativePath = record.path.startsWith(sedonaPrefix) ? record.path.slice(sedonaPrefix.length) : record.path;
      const sourceFile = path.join(sedonaDir, relativePath);
   ...
   * @param {string} projectDir - Project directory
   */
  async cleanup(projectDir, options = {}, sedonaDir = null) {
    const resolvedBmadDir = sedonaDir || (await this._findBmadDir(projectDir));
    ...
    // Strip SEDONA markers from copilot-instructions.md if present
    if (this.name === 'github-copilot') {
      await this.cleanupCopilotInstructions(projectDir, options);
    }

    // Strip SEDONA modes from .kilocodemodes if present
    if (this.name === 'kilo') {
      await this.cleanupKiloModes(projectDir, options);
    }

    // Strip SEDONA entries from .rovodev/prompts.yml if present
  ...
  /**
   * Warn about stale SEDONA files in a global legacy directory (never auto-deletes)
  ...
  async warnGlobalLegacy(legacyDir, options = {}) {
    try {
      ...
      const sedonaFiles = entries.filter((e) => typeof e === 'string' && e.startsWith('sedona'));

      if (sedonaFiles.length > 0 && !options.silent) {
        await prompts.log.warn(`Found ${sedonaFiles.length} stale SEDONA file(s) in ${expanded}. Remove manually: rm ${expanded}/sedona-*`);
      }
    } catch {
  ...
  /**
   * Find the _sedona directory in a project
   * @param {string} projectDir - Project directory
   * @returns {string|null} Path to sedona dir or null
   */
  async _findBmadDir(projectDir) {
    const sedonaDir = path.join(projectDir, SEDONA_FOLDER_NAME);
    return (await fs.pathExists(sedonaDir)) ? sedonaDir : null;
  }
   ...
   */
  async _buildUninstallSet(sedonaDir) {
    const removals = await this.loadRemovalLists(sedonaDir);
  ...
  /**
   * Load removal lists from all module sources in the sedona directory.
   * Each module can have an optional removals.txt listing entries to remove.
   * @param {string} sedonaDir - SEDONA installation directory
   * @returns {Set<string>} Set of entries to remove
   */
  async loadRemovalLists(sedonaDir) {
    ...
    try {
      const entries = await fs.readdir(sedonaDir);
      for (const entry of entries) {
        if (entry.startsWith('_')) continue;
        const removalPath = path.join(sedonaDir, entry, 'removals.txt');
        await this._readRemovalFile(removalPath, removals);
      }
    } catch {
      // sedonaDir may not exist yet on fresh install
    }
   ...
   * When removalSet is null (legacy dirs), removes all sedona-prefixed entries.
   * @param {string} projectDir - Project directory
  ...
  async cleanupTarget(projectDir, targetDir, options = {}, removalSet = new Set()) {
    ...
    for (const entry of entries) {
      ...
      // Always preserve sedona-os-* utility skills regardless of cleanup mode
      if (entry.startsWith('sedona-os-')) continue;

      // Surgical removal from set, or legacy prefix matching when set is null
      const shouldRemove = removalSet ? removalSet.has(entry) : entry.startsWith('sedona');
  ...
  /**
   * Strip SEDONA-owned content from .github/copilot-instructions.md.
   * The old custom installer injected content between <!-- SEDONA:START --> and <!-- SEDONA:END --> markers.
  ...
  async cleanupCopilotInstructions(projectDir, options = {}) {
    ...
    try {
      const content = await fs.readFile(filePath, 'utf8');
      const startIdx = content.indexOf('<!-- SEDONA:START -->');
      const endIdx = content.indexOf('<!-- SEDONA:END -->');

      if (startIdx === -1 || endIdx === -1 || endIdx <= startIdx) return;

      const cleaned = content.slice(0, startIdx) + content.slice(endIdx + '<!-- SEDONA:END -->'.length);
      ...
      if (!options.silent) await prompts.log.message('  Cleaned SEDONA markers from copilot-instructions.md');
    } catch {
      if (!options.silent) await prompts.log.warn('  Warning: Could not clean SEDONA markers from copilot-instructions.md');
    }
  }

  /**
   * Strip SEDONA-owned modes from .kilocodemodes.
   * The old custom kilo.js installer added modes with slug starting with 'sedona-'.
   * Parses YAML, filters out SEDONA modes, rewrites. Leaves file as-is on parse failure.
   */
  async cleanupKiloModes(projectDir, options = {}) {
    ...
    config.customModes = config.customModes.filter((mode) => mode && (!mode.slug || !mode.slug.startsWith('sedona-')));
    ...
    if (removedCount > 0) {
      try {
        ...
        if (!options.silent) await prompts.log.message(`  Removed ${removedCount} SEDONA modes from .kilocodemodes`);
      } catch {
  ...
  /**
   * Strip SEDONA-owned entries from .rovodev/prompts.yml.
   * The old custom rovodev.js installer registered workflows in prompts.yml.
   * Parses YAML, filters out entries with name starting with 'sedona-', rewrites.
  ...
  async cleanupRovoDevPrompts(projectDir, options = {}) {
    ...
    config.prompts = config.prompts.filter((entry) => entry && (!entry.name || !entry.name.startsWith('sedona-')));
    ...
    if (removedCount > 0) {
      try {
        ...
        if (!options.silent) await prompts.log.message(`  Removed ${removedCount} SEDONA entries from prompts.yml`);
      } catch {
  ...
  /**
   * Check ancestor directories for existing SEDONA files in the same target_dir.
      ...
      try {
        if (await fs.pathExists(candidatePath)) {
          ...
          const hasBmad = entries.some(
            (e) => typeof e === 'string' && e.toLowerCase().startsWith('sedona') && !e.toLowerCase().startsWith('sedona-os-'),
          );
          if (hasBmad) {
