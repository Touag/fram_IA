...
class IdeManager {
  constructor() {
    ...
    this.sedonaFolderName = SEDONA_FOLDER_NAME; // Default, can be overridden
  }

  /**
   * Set the sedona folder name for all IDE handlers
   * @param {string} sedonaFolderName - The sedona folder name
   */
  setBmadFolderName(sedonaFolderName) {
    ...
    for (const handler of this.handlers.values()) {
      if (typeof handler.setBmadFolderName === 'function') {
        handler.setBmadFolderName(sedonaFolderName);
      }
    }
  }
  ...
  async loadConfigDrivenHandlers() {
    ...
    for (const [platformCode, platformInfo] of Object.entries(platformConfig.platforms)) {
      ...
      if (typeof handler.setBmadFolderName === 'function') {
        handler.setBmadFolderName(this.sedonaFolderName);
      }
   ...
   */
  async setup(ideName, projectDir, sedonaDir, options = {}) {
    ...
    try {
      const handlerResult = await handler.setup(projectDir, sedonaDir, options);
