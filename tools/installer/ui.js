...
async function getMarketplaceVersion(moduleCode) {
  ...
  if (moduleCode === 'core' || moduleCode === 'bmm') {
    ...
    const cacheDir = path.join(os.homedir(), '.sedona', 'cache', 'external-modules', moduleCode);
...
class UI {
  ...
  async promptInstall(options = {}) {
    ...
    const { sedonaDir } = await installer.findBmadDir(confirmedDirectory);

    // Check if there's an existing SEDONA installation
    const hasExistingInstall = await fs.pathExists(sedonaDir);
    ...
    if (hasExistingInstall) {
      ...
      const { existingInstall, sedonaDir } = await this.getExistingInstallation(confirmedDirectory);
      ...
      // Common actions
      choices.push({ name: 'Modify SEDONA Installation', value: 'update' });
  ...
  async promptToolSelection(projectDir, options = {}) {
    ...
    const { Installer } = require('./core/installer');
    const installer = new Installer();
    const { sedonaDir } = await installer.findBmadDir(projectDir || process.cwd());
   ...
   * @returns {Object} Object with existingInstall, installedModuleIds, and sedonaDir
   */
  async getExistingInstallation(directory) {
    const { ExistingInstall } = require('./core/existing-install');
    const { Installer } = require('./core/installer');
    const installer = new Installer();
    const { sedonaDir } = await installer.findBmadDir(directory);
    const existingInstall = await ExistingInstall.detect(sedonaDir);
    const installedModuleIds = new Set(existingInstall.moduleIds);

    return { existingInstall, installedModuleIds, sedonaDir };
  }
  ...
  async _browseCommunityModules(installedModuleIds = new Set()) {
    ...
    while (browsing) {
      ...
      const trustBadge = (tier) => {
        if (tier === 'sedona-certified') return '\u2713';
    ...
    while (addMore) {
      ...
      if (sourceResult.parsed.type === 'local') {
        ...
        await prompts.log.warn(
          'UNVERIFIED MODULE: This module has not been reviewed by the Sedona team.\n' + '  Only install modules from sources you trust.',
        );
      }
  ...
  async displayDirectoryInfo(directory) {
    ...
    if (dirExists) {
      ...
      if (stats.isDirectory()) {
        ...
        if (files.length > 0) {
          // Check for any sedona installation (any folder with _config/manifest.yaml)
          const { Installer } = require('./core/installer');
          const installer = new Installer();
          const sedonaResult = await installer.findBmadDir(directory);
          const hasBmadInstall =
            (await fs.pathExists(sedonaResult.sedonaDir)) && (await fs.pathExists(path.join(sedonaResult.sedonaDir, '_config', 'manifest.yaml')));

          const sedonaNote = hasBmadInstall ? ` including existing SEDONA installation (${path.basename(sedonaResult.sedonaDir)})` : '';
          await prompts.log.message(`Directory exists and contains ${files.length} item(s)${sedonaNote}`);
        } else {
  ...
  async displayStatus(statusData) {
    const { installation, modules, availableUpdates, sedonaDir } = statusData;
    ...
    if (availableUpdates.length > 0) {
      ...
      await prompts.log.message('Run \'sedona install\' and select "Quick Update" to update');
    } else {
