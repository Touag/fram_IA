...
module.exports = {
  command: 'uninstall',
  description: 'Remove SEDONA installation from the current project',
  options: [
    ['-y, --yes', 'Remove all SEDONA components without prompting (preserves user artifacts)'],
    ...
    try {
      ...
      if (options.directory) {
        ...
        const dirChoice = await prompts.select({
          message: 'Where do you want to uninstall SEDONA from?',
          choices: [
      ...
      if (!(await fs.pathExists(sedonaDir))) {
        await prompts.log.warn('No SEDONA installation found.');
        process.exit(0);
      }
      ...
      await prompts.intro('SEDONA Uninstall');
      ...
      if (!options.yes) {
        ...
        const selected = await prompts.multiselect({
          ...
          options: [
            {
              value: 'modules',
              label: `SEDONA Modules & data (${installer.sedonaFolderName}/)`,
      ...
      // Phase 3: SEDONA modules & data (last — other phases may need _sedona/)
      if (removeModules) {
        const s = await prompts.spinner();
        s.start(`Removing SEDONA modules & data (${installer.sedonaFolderName}/)...`);
      ...
      await prompts.outro('To reinstall, run: npx sedona install');
