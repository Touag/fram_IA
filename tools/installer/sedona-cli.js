  ...
  try {
    ...
    if (result && semver.gt(result, packageJson.version)) {
      ...
      const updateMsg = [
        ...
        `  npm cache clean --force && npx sedona@${tag} install`,
      ].join('\n');
...
// Set up main program
program.version(packageJson.version).description('SEDONA Core CLI - Universal AI agent framework');
