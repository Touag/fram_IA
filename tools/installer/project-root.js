...
/**
 * Find the SEDONA project root directory by looking for package.json
 * or specific SEDONA markers
 */
function findProjectRoot(startPath = __dirname) {
  let currentPath = path.resolve(startPath);

  // Keep going up until we find package.json with sedona
  while (currentPath !== path.dirname(currentPath)) {
    ...
    if (fs.existsSync(packagePath)) {
      try {
        ...
        // Check if this is the SEDONA project
        if (pkg.name === 'sedona' || fs.existsSync(path.join(currentPath, 'src', 'core-skills'))) {
          return currentPath;
        }
      } catch {
        // Continue searching
