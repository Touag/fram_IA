...
/**
 * Immutable snapshot of an existing SEDONA installation.
...
class ExistingInstall {
  ...
  /**
   * Scan a sedona directory and return an immutable snapshot of what's installed.
   * @param {string} sedonaDir - Path to sedona directory
   * @returns {Promise<ExistingInstall>}
   */
  static async detect(sedonaDir) {
    ...
    const manifestData = await manifest.read(sedonaDir);
    if (manifestData) {
    ...
    const corePath = path.join(sedonaDir, 'core');
    ...
    if (manifestData && manifestData.modules && manifestData.modules.length > 0) {
      for (const moduleId of manifestData.modules) {
        const modulePath = path.join(sedonaDir, moduleId);
