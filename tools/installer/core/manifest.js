...
class Manifest {
   ...
   */
  async create(sedonaDir, data, installedFiles = []) {
    ...
    // Get the Sedona version from package.json
    const sedonaVersion = data.version || require(path.join(process.cwd(), 'package.json')).version;

    // Convert module list to new detailed format
    const moduleDetails = [];
    if (data.modules && Array.isArray(data.modules)) {
      for (const moduleName of data.modules) {
        // Core and BMM modules use the Sedona version
        const moduleVersion = moduleName === 'core' || moduleName === 'bmm' ? sedonaVersion : null;
   ...
   */
  async read(sedonaDir) {
    const yamlPath = path.join(sedonaDir, '_config', 'manifest.yaml');
    const yaml = require('yaml');
   ...
   */
  async addModule(sedonaDir, moduleName, options = {}) {
    let manifest = await this._readRaw(sedonaDir);
    if (!manifest) {
    ...
    await this._writeRaw(sedonaDir, manifest);
  }
   ...
   */
  async getAllModuleVersions(sedonaDir) {
    const manifest = await this._readRaw(sedonaDir);
   ...
   */
  async _writeRaw(sedonaDir, manifestData) {
    const yaml = require('yaml');
    const manifestPath = path.join(sedonaDir, '_config', 'manifest.yaml');
   ...
   */
  async getModuleVersionInfo(moduleName, sedonaDir, moduleSourcePath = null) {
    const yaml = require('yaml');
    ...
    const customSource = await customMgr.findModuleSourceByCode(moduleName, { sedonaDir });
  ...
  async _readMarketplaceVersion(moduleName, moduleSourcePath = null) {
    ...
    if (!marketplacePath) {
      const cacheDir = path.join(os.homedir(), '.sedona', 'cache', 'external-modules', moduleName);
   ...
   */
  async checkForUpdates(sedonaDir) {
    const modules = await this.getAllModuleVersions(sedonaDir);
    const updates = [];
