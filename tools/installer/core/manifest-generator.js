...
class ManifestGenerator {
   ...
   * @param {string} sedonaDir - _sedona
   ...
   */
  async generateManifests(sedonaDir, selectedModules, installedFiles = [], options = {}) {
    // Create _config directory if it doesn't exist
    const cfgDir = path.join(sedonaDir, '_config');
    await fs.ensureDir(cfgDir);

    // Store modules list (all modules including preserved ones)
    const preservedModules = options.preservedModules || [];

    // Scan the sedona directory to find all actually installed modules
    const installedModules = await this.scanInstalledModules(sedonaDir);
    ...
    this.sedonaDir = sedonaDir;
    this.sedonaFolderName = path.basename(sedonaDir); // Get the actual folder name (e.g., '_sedona' or 'sedona')
  ...
  async collectSkills() {
    ...
    const debug = process.env.SEDONA_DEBUG_MANIFEST === 'true';

    for (const moduleName of this.updatedModules) {
      const modulePath = path.join(this.sedonaDir, moduleName);
      ...
      const walk = async (dir) => {
        ...
        if (skillMeta) {
          ...
          const installPath = relativePath
            ? `${this.sedonaFolderName}/${moduleName}/${relativePath}/${skillFile}`
            : `${this.sedonaFolderName}/${moduleName}/${skillFile}`;
          ...
          if (manifest && manifest.__single && manifest.__single.canonicalId && artifactType !== 'agent') {
            console.warn(
              `Warning: Native entrypoint manifest at ${dir}/sedona-skill-manifest.yaml contains canonicalId — this field is ignored for SKILL.md directories (directory name is the canonical ID)`,
            );
          }
  ...
  async collectAgents(selectedModules) {
    ...
    // Get standalone agents from sedona/agents/ directory
    const standaloneAgentsDir = path.join(this.sedonaDir, 'agents');
   ...
   * Discovers agents via directory with sedona-skill-manifest.yaml containing type: agent
   *
  ...
  async getAgentsFromDirRecursive(dirPath, moduleName, relativePath = '', debug = false) {
    ...
    for (const entry of entries) {
      ...
      if (dirManifest && dirManifest.__single && dirManifest.__single.type === 'agent') {
        ...
        const installPath = `${this.sedonaFolderName}/${agentModule}/${dirRelativePath}`;
  ...
  async writeMainManifest(cfgDir) {
    ...
    for (const moduleName of this.modules) {
      ...
      const versionInfo = await manifestObj.getModuleVersionInfo(moduleName, this.sedonaDir);
  ...
  async writeFilesManifest(cfgDir) {
    ...
    if (this.allInstalledFiles && this.allInstalledFiles.length > 0) {
      ...
      for (const filePath of this.allInstalledFiles) {
        // Store paths relative to sedonaDir (no folder prefix)
        const relativePath = filePath.replace(this.sedonaDir, '').replaceAll('\\', '/').replace(/^\//, '');
      ...
      for (const file of this.files) {
        ...
        const relPath = file.path.replace(this.sedonaFolderName + '/', '');
        const filePath = path.join(this.sedonaDir, relPath);
        const hash = await this.calculateFileHash(filePath);
        allFiles.push({
          ...file,
          path: relPath,
  ...
  /**
   * Scan the sedona directory to find all installed modules
   * @param {string} sedonaDir - Path to sedona directory
   * @returns {Array} List of module names
   */
  async scanInstalledModules(sedonaDir) {
    const modules = [];
    ...
    try {
      ...
      for (const entry of entries) {
        ...
        const modulePath = path.join(sedonaDir, entry.name);
