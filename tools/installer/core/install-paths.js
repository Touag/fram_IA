...
class InstallPaths {
  static async create(config) {
    ...
    await assertReadableDir(srcDir, 'SEDONA source root');
    ...
    const sedonaDir = path.join(projectRoot, SEDONA_FOLDER_NAME);
    const isUpdate = await fs.pathExists(sedonaDir);

    const configDir = path.join(sedonaDir, '_config');
    const agentsDir = path.join(configDir, 'agents');
    const coreDir = path.join(sedonaDir, 'core');

    for (const [dir, label] of [
      [sedonaDir, 'sedona directory'],
  ...
  helpCatalog() {
    return path.join(this.configDir, 'sedona-help.csv');
  }
  moduleDir(name) {
    return path.join(this.sedonaDir, name);
  }
  moduleConfig(name) {
    return path.join(this.sedonaDir, name, 'config.yaml');
  }
}
