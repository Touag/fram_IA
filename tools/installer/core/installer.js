...
const { SEDONA_FOLDER_NAME } = require('../ide/shared/path-utils');
...
class Installer {
  constructor() {
    ...
    this.sedonaFolderName = SEDONA_FOLDER_NAME;
  }
  ...
  async install(originalConfig) {
    ...
    try {
      ...
      const existingInstall = await ExistingInstall.detect(paths.sedonaDir);
      ...
      if (existingInstall.installed) {
        const existingModules = await this.manifest.getAllModuleVersions(paths.sedonaDir);
      ...
      const prevCsvPath = path.join(paths.sedonaDir, '_config', 'skill-manifest.csv');
  ...
  async _validateIdeSelection(config) {
    ...
    if (suspendedIdes.length > 0 && suspendedIdes.length === config.ides.length) {
      ...
      throw new Error(
        `All selected tool(s) are suspended: ${suspendedIdes.join(', ')}. Installation aborted to prevent upgrading _sedona/ without a working IDE configuration.`,
      );
    }
  }
  ...
  async _installAndConfigure(config, originalConfig, paths, officialModuleIds, allModules, addResult, officialModules) {
    ...
    installTasks.push({
      ...
      task: async (message) => {
        const verboseMode = process.env.SEDONA_VERBOSE_INSTALL === 'true' || config.verbose;
        const moduleLogger = {
        ...
        if (config.modules && config.modules.length > 0) {
          for (const moduleName of config.modules) {
            ...
            const result = await officialModules.createModuleDirectories(moduleName, paths.sedonaDir, {
    ...
    const configTask = {
      ...
      task: async (message) => {
        await this.generateModuleConfigs(paths.sedonaDir, moduleConfigs);
        ...
        await manifestGen.generateManifests(paths.sedonaDir, allModulesForManifest, [...this.installedFiles], {
          ides: config.ides || [],
          preservedModules: modulesForCsvPreserve,
        });

        message('Generating help catalog...');
        await this.mergeModuleHelpCatalogs(paths.sedonaDir);
  ...
  async _setupIdes(config, allModules, paths, addResult, previousSkillIds = new Set()) {
    ...
    for (const ide of validIdes) {
      const setupResult = await this.ideManager.setup(ide, paths.projectRoot, paths.sedonaDir, {
  ...
  /**
   * Remove skill directories from _sedona/ after IDE installation.
   * Skills are self-contained in IDE directories, so _sedona/ only needs
   * module-level files (config.yaml, _config/, etc.).
   * Also cleans up skill dirs left by older installer versions.
   * @param {string} sedonaDir - SEDONA installation directory
   */
  async _cleanupSkillDirs(sedonaDir) {
    const csv = require('csv-parse/sync');
    const csvPath = path.join(sedonaDir, '_config', 'skill-manifest.csv');
    if (!(await fs.pathExists(csvPath))) return;

    const csvContent = await fs.readFile(csvPath, 'utf8');
    const records = csv.parse(csvContent, { columns: true, skip_empty_lines: true });
    const sedonaFolderName = path.basename(sedonaDir);
    const sedonaPrefix = sedonaFolderName + '/';

    for (const record of records) {
      if (!record.path) continue;
      const relativePath = record.path.startsWith(sedonaPrefix) ? record.path.slice(sedonaPrefix.length) : record.path;
      const sourceDir = path.dirname(path.join(sedonaDir, relativePath));
        ...
        task: async (message) => {
          if (updateState.customFiles.length > 0) {
            ...
            for (const originalPath of updateState.customFiles) {
              const relativePath = path.relative(paths.sedonaDir, originalPath);
          ...
          if (updateState.modifiedFiles.length > 0) {
            ...
            if (updateState.tempModifiedBackupDir && (await fs.pathExists(updateState.tempModifiedBackupDir))) {
              ...
              for (const modifiedFile of restoredModifiedFiles) {
                const relativePath = path.relative(paths.sedonaDir, modifiedFile.path);
  ...
  async _prepareUpdateState(paths, config, existingInstall, officialModules) {
    ...
    const existingFilesManifest = await this.readFilesManifest(paths.sedonaDir);
    const { customFiles, modifiedFiles } = await this.detectCustomFiles(paths.sedonaDir, existingFilesManifest);
  ...
  async _backupUserFiles(paths, customFiles, modifiedFiles) {
    ...
    if (customFiles.length > 0) {
      tempBackupDir = path.join(paths.projectRoot, '_sedona-custom-backup-temp');
      await fs.ensureDir(tempBackupDir);

      for (const customFile of customFiles) {
        const relativePath = path.relative(paths.sedonaDir, customFile);
    ...
    if (modifiedFiles.length > 0) {
      tempModifiedBackupDir = path.join(paths.projectRoot, '_sedona-modified-backup-temp');
   ...
   */
  async readFilesManifest(sedonaDir) {
    const filesManifestPath = path.join(sedonaDir, '_config', 'files-manifest.csv');
   ...
   */
  async detectCustomFiles(sedonaDir, existingFilesManifest) {
    const customFiles = [];
    const modifiedFiles = [];

    // Memory is always in _sedona/_memory
    const sedonaMemoryPath = '_memory';
    ...
    for (const fileEntry of existingFilesManifest) {
      if (fileEntry.path) {
        const absolutePath = path.join(sedonaDir, fileEntry.path);
    ...
    // Recursively scan sedonaDir for all files
      ...
      try {
        ...
        for (const entry of entries) {
          ...
          if (entry.isDirectory()) {
            ...
            const relativePath = path.relative(sedonaDir, fullPath);
            ...
            if (relativePath.startsWith('_config/') || relativePath.startsWith('_config\\')) {
              ...
              if (relativePath.includes('/agents/') && fileName.endsWith('.customize.yaml')) {
                ...
                const manifestPath = path.join(sedonaDir, '_config', 'manifest.yaml');
            ...
            if (relativePath.startsWith(sedonaMemoryPath + '/') && path.dirname(relativePath).includes('-sidecar')) {
              continue;
            }
   ...
   */
  async generateModuleConfigs(sedonaDir, moduleConfigs) {
    const yaml = require('yaml');
    ...
    const entries = await fs.readdir(sedonaDir, { withFileTypes: true });
    ...
    for (const moduleName of installedModules) {
      const modulePath = path.join(sedonaDir, moduleName);
...
`;
  ...
  /**
   * Merge all module-help.csv files into a single sedona-help.csv
   * Scans all installed modules for module-help.csv and merges them
   * Enriches agent info from agent-manifest.csv
   * Output is written to _sedona/_config/sedona-help.csv
   * @param {string} sedonaDir - SEDONA installation directory
   */
  async mergeModuleHelpCatalogs(sedonaDir) {
    const allRows = [];
    const headerRow =
      'module,phase,name,code,sequence,workflow-file,command,required,agent-name,agent-command,agent-display-name,agent-title,options,description,output-location,outputs';

    // Load agent manifest for agent info lookup
    const agentManifestPath = path.join(sedonaDir, '_config', 'agent-manifest.csv');
    ...
    if (await fs.pathExists(agentManifestPath)) {
      ...
      for (const line of lines) {
        ...
        if (cols.length >= 4) {
          ...
          // Build agent command: sedona:module:agent:name
          const agentCommand = module ? `sedona:${module}:agent:${agentName}` : `sedona:agent:${agentName}`;
    ...
    for (const [moduleName, modulePath] of modulePaths) {
      ...
      if (await fs.pathExists(helpFilePath)) {
        try {
          ...
          if (process.env.SEDONA_VERBOSE_INSTALL === 'true') {
    ...
    const outputDir = path.join(sedonaDir, '_config');
    await fs.ensureDir(outputDir);
    const outputPath = path.join(outputDir, 'sedona-help.csv');
    ...
    if (process.env.SEDONA_VERBOSE_INSTALL === 'true') {
      await prompts.log.message(`  Generated sedona-help.csv: ${allRows.length} workflows`);
    }
  }
   ...
   * @param {Object} context - {sedonaDir, modules, ides, customFiles, modifiedFiles}
   */
  async renderInstallSummary(results, context = {}) {
    ...
    if ((context.ides || []).length === 0) {
      lines.push(`  ${color.green('\u2713')}  No IDE selected (installed in _sedona only)`);
    }

    // Context and warnings
    lines.push('');
    if (context.sedonaDir) {
      lines.push(`  Installed to: ${context.sedonaDir}`);
    }
    ...
    lines.push(
      ...
      `    2. Not sure what to do? Invoke the ${color.cyan('sedona-help')} skill and ask it what to do!`,
      '',
      `    GitHub: ${color.blue('https://github.com/Touag/fram_IA')}`,
    );

    await prompts.box(lines.join('\n'), 'SEDONA is ready to use!', {
      rounded: true,
  ...
  async quickUpdate(config) {
    ...
    const { sedonaDir } = await this.findBmadDir(projectDir);

    // Check if sedona directory exists
    if (!(await fs.pathExists(sedonaDir))) {
      throw new Error(`SEDONA not installed at ${sedonaDir}. Use regular install for first-time setup.`);
    }

    // Detect existing installation
    const existingInstall = await ExistingInstall.detect(sedonaDir);
    const installedModules = existingInstall.moduleIds;
    const configuredIdes = existingInstall.ides;
    const projectRoot = path.dirname(sedonaDir);
    ...
    for (const moduleId of installedModules) {
      if (!availableModules.some((m) => m.id === moduleId)) {
        const customSource = await customMgr.findModuleSourceByCode(moduleId, { sedonaDir });
        if (customSource) {
          availableModules.push({
            id: moduleId,
            name: moduleId,
  ...
  /**
   * Uninstall SEDONA with selective removal options
   * @param {string} directory - Project directory
   * @param {Object} options - Uninstall options
   * @param {boolean} [options.removeModules=true] - Remove _sedona/ directory
  ...
  async uninstall(directory, options = {}) {
    ...
    const existingInstall = await ExistingInstall.detect(sedonaDir);
    const outputFolder = await this._readOutputFolder(sedonaDir);

    const removed = { modules: false, ideConfigs: false, outputFolder: false };

    // 2. IDE CLEANUP (before _sedona/ deletion so configs are accessible)
    ...
    // 4. SEDONA DIRECTORY (last, after everything that needs it)
  ...
  async getStatus(directory) {
    const projectDir = path.resolve(directory);
    const { sedonaDir } = await this.findBmadDir(projectDir);
    return await ExistingInstall.detect(sedonaDir);
  }
   ...
   * Resolves sedonaDir internally from projectDir
   * @param {string} projectDir - Project directory
   * @returns {string} Output folder name (relative, default: '_sedona-output')
   */
  async getOutputFolder(projectDir) {
    const { sedonaDir } = await this.findBmadDir(projectDir);
    return this._readOutputFolder(sedonaDir);
  }

  /**
   * Find the sedona installation directory in a project
   * Always uses the standard _sedona folder name
   * @param {string} projectDir - Project directory
   * @returns {Promise<Object>} { sedonaDir: string }
   */
  async findBmadDir(projectDir) {
    const sedonaDir = path.join(projectDir, SEDONA_FOLDER_NAME);
    return { sedonaDir };
  }
   ...
   */
  async _readOutputFolder(sedonaDir) {
    const yaml = require('yaml');

    // Check bmm/config.yaml first (most common)
    const bmmConfigPath = path.join(sedonaDir, 'bmm', 'config.yaml');
    ...
    try {
      ...
      for (const entry of entries) {
        ...
        const configPath = path.join(sedonaDir, entry.name, 'config.yaml');
