...
description: Corrects workflow, agent, and command references in Sedona documentation
---
...
Fix incorrect references to workflows, agents, and commands in Sedona documentation files.
...
**How-To guides require judgment**: Don't assume experienced. Ask: "Does this task require prior Sedona knowledge?" Early-journey tasks (first PRD, first sprint) are newbie docs. Customization and advanced features are experienced.

**If unclear**: Ask the user "Who is the target audience for this document — new users learning Sedona, or experienced users who know the system?"
...
| `sedona_bmm_*` | `sedona_bmm_workflow-init` | Internal slash command name, platform-specific |
...
- First mention: Run the `help` workflow (`sedona-help` on most platforms)
...
|----------|----------|-------|
| `workflow-init` | `sedona-help` | DEPRECATED - help system replaces initialization |
| `workflow-status` | `sedona-help` | DEPRECATED - help system replaces status checking |

### The Help System

The `sedona-help` workflow is the modern replacement for both `workflow-init` and `workflow-status`:
...
Users can run `sedona-help` to get guidance on what to do next. It detects:
...
3. **Simple names**: Just the workflow name, no `sedona_bmm_` prefix, no `/` prefix
