 ...
 *
 * Validates cross-file references in SEDONA source files (agents, workflows, tasks, steps).
 * Catches broken file paths, missing referenced files, and absolute path leaks.
 *
 * What it checks:
 * - {project-root}/_sedona/ references in YAML and markdown resolve to real src/ files
...
// Pattern: {project-root}/_sedona/ references
const PROJECT_ROOT_REF = /\{project-root\}\/_sedona\/([^\s'"<>})\]`]+)/g;

// Pattern: {_sedona}/ shorthand references
const SEDONA_SHORTHAND_REF = /\{_sedona\}\/([^\s'"<>})\]`]+)/g;
...
function mapInstalledToSource(refPath) {
  // Strip {project-root}/_sedona/ or {_sedona}/ prefix
  let cleaned = refPath.replace(/^\{project-root\}\/_sedona\//, '').replace(/^\{_sedona\}\//, '');

  // Also handle bare _sedona/ prefix (seen in some invoke-task)
  cleaned = cleaned.replace(/^_sedona\//, '');
  ...
  // _sedona/core/ → src/core-skills/, _sedona/bmm/ → src/bmm-skills/
...
function isInstallOnly(cleanedPath) {
  // Skip paths that only exist in the installed _sedona/ structure, not in src/
...
function extractYamlRefs(filePath, content) {
  ...
  function checkValue(value, range, keyPath) {
    ...
    // Check for {project-root}/_sedona/ refs
    const prMatch = value.match(/\{project-root\}\/_sedona\/[^\s'"<>})\]`]+/);
    if (prMatch) {
      refs.push({ file: filePath, raw: prMatch[0], type: 'project-root', line, key: keyPath });
    }

    // Check for {_sedona}/ refs
    const bmMatch = value.match(/\{_sedona\}\/[^\s'"<>})\]`]+/);
    if (bmMatch) {
...
function extractMarkdownRefs(filePath, content) {
  ...
  // {project-root}/_sedona/ refs
  runPattern(PROJECT_ROOT_REF, 'project-root');

  // {_sedona}/ shorthand
  runPattern(SEDONA_SHORTHAND_REF, 'project-root');
...
function resolveRef(ref) {
  ...
  if (ref.type === 'exec-attr') {
    ...
    if (execPath.includes('{_sedona}')) {
      return mapInstalledToSource(execPath);
    }
    if (execPath.startsWith('_sedona/')) {
      return mapInstalledToSource(execPath);
    }
    // Relative exec path
  ...
  if (ref.type === 'invoke-task') {
    ...
    const prMatch = ref.raw.match(/\{project-root\}\/_sedona\/([^\s'"<>})\]`]+)/);
    if (prMatch) return mapInstalledToSource(prMatch[0]);

    const bmMatch = ref.raw.match(/\{_sedona\}\/([^\s'"<>})\]`]+)/);
    if (bmMatch) return mapInstalledToSource(bmMatch[0]);

    const bareMatch = ref.raw.match(/_sedona\/([^\s'"<>})\]`]+)/);
